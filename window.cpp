...
Window::Window (short get_x, short get_y, short get_w, short get_h, string get_title, short get_special_input,
                image_data* get_background_image) {
    x = get_x;
    y = get_y;
    w = get_w;
    h = get_h;
...
void Window::turn_off () {
    ...
    // The reason is that this command is needed to ensure that whenever the user closes the key config window, the game
    // stops looking for a
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (creating_profile.length() < 16) {
                if ((text[i] >= (Uint16) '0' && text[i] <= (Uint16) '9') ||
                    ...
                    (text[i] >= (Uint16) 'a' && text[i] <= (Uint16) 'z') || text[i] == (Uint16) '_' ||
                    text[i] == (Uint16) '-' || text[i] == (Uint16) '.') {
...
void Window::create_button (short b_x, short b_y, string b_tooltip_text, string b_text,
                            void (*b_event_function) (Window*, int), sound_data* get_mouse_over_sound,
                            sound_data* get_event_fire_sound, bool get_invisible, int get_special_data,
                            bool get_enabled) {
    buttons.push_back(Button(b_x, b_y, b_tooltip_text, b_text, b_event_function, get_mouse_over_sound,
                             get_event_fire_sound, get_invisible, get_special_data, get_enabled));
}

void Window::create_information (short get_x, short get_y, string get_tooltip_text, string get_text,
                                 image_data* get_info_image, short get_special_info, SDL_Rect* get_info_sprite_sheet,
                                 short get_text_color, short get_font_type) {
    informations.push_back(Information(get_x, get_y, get_tooltip_text, get_text, get_info_image, get_special_info,
                                       get_info_sprite_sheet, get_text_color, get_font_type));
}
