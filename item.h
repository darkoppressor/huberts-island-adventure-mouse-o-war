...
class Item {
    private:
        ...
        // Simply tells us whether or not the item is currently in the air. If so, the item's y coordinate will be
        // incremented by air_velocity.
        // Also, the movement code, event code, etc. only runs when the item is in the air. Once it lands on the ground
        // somewhere, it no longer moves or handles events.
        bool IN_AIR;
    ...
    public:
        ...
        Item (double get_x, double get_y, bool get_IN_AIR, short get_type, int get_goal_level_to_load,
              bool get_goal_secret, int min_move = 10, int max_move = 20, int min_jump = 20, int max_jump = 40,
              bool can_start_falling = false, short get_counter_vacuum = 0, int get_score_bonus = 0);
        ...
        // These two variables let the game know if the checkpoint was just reached or deactivated (a different
