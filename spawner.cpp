...
Spawner::Spawner (short get_spawner_type, short get_type, double get_x, double get_y, bool get_disallow_doubles,
                  bool get_items_stay) {
    ...
    if (disallow_doubles) {
        ...
        if (spawner_type == SPAWN_ITEM) {
            for (int i = 0; i < vector_items.size(); i++) {
                if (vector_items[i].exists && type == vector_items[i].type && collision_check(x, y, ITEM_W, ITEM_H,
                                                                                              vector_items[i].x,
                                                                                              vector_items[i].y,
                                                                                              vector_items[i].w,
                                                                                              vector_items[i].h)) {
            ...
            for (int i = 0; i < vector_npcs.size(); i++) {
                if (vector_npcs[i].exists && type == vector_npcs[i].type && collision_check(x, y, ITEM_W, ITEM_H,
                                                                                            vector_npcs[i].x,
                                                                                            vector_npcs[i].y,
                                                                                            vector_npcs[i].w,
                                                                                            vector_npcs[i].h)) {
