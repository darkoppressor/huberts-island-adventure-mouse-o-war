...
void Tile::render () {
    ...
    if (x >= player.camera_x - TILE_SIZE && x <= player.camera_x + player.camera_w &&
        y >= player.camera_y - TILE_SIZE && y <= player.camera_y + player.camera_h) {
        ...
        if (sprite != 0) {
                ...
                if (sprite == 1 || sprite == 2) {
                    if (sprite == 2) {
                        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                      image.water_tiles, &worldmap_sprites_tiles[sprite], 0.75, scale_x, scale_y);
                    } else {
                        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                      image.water_tiles, &sprites_water_tiles[player.frame_water], 0.75, scale_x,
                                      scale_y);
                    }
                } else {
                    render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                  image.tiles, &worldmap_sprites_tiles[sprite], 1.0, scale_x, scale_y);
                }
            }
            // If the player is in a level.
                ...
                if (special == TILE_SPECIAL_WATER) {
                    ...
                    if (sprite == 2) {
                        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                      image.water_tiles, &sprites_tiles[sprite], water_opacity);
                    } else {
                        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                      image.water_tiles, &sprites_water_tiles[player.frame_water], water_opacity);
                    }
                } else {
                    double opacity = 1.0;
                    ...
                    render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                  image.tiles, &sprites_tiles[sprite], opacity);
                }
            }
