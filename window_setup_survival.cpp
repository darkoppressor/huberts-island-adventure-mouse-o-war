...
Window_Setup_Survival::Window_Setup_Survival (short get_x, short get_y, short get_w, short get_h, string get_title) {
    ...
    button_sound = 4 - 1;
    y_position = 70;
    create_button(button_spacing, y_position, "", msg, &button_event_survival_time_very_short,
                  &sound_system.button_mouse_over[button_sound], &sound_system.button_event_fire[button_sound],
                  BUTTON_VISIBLE);
    ...
    button_sound = 6 - 1;
    y_position = 70;
    create_button(button_spacing, y_position, "", msg, &button_event_survival_time_short,
                  &sound_system.button_mouse_over[button_sound], &sound_system.button_event_fire[button_sound],
                  BUTTON_VISIBLE);
    button_spacing += buttons[buttons.size() - 1].w + 30;
    ...
    button_sound = 8 - 1;
    y_position = 70;
    create_button(button_spacing, y_position, "", msg, &button_event_survival_time_medium,
                  &sound_system.button_mouse_over[button_sound], &sound_system.button_event_fire[button_sound],
                  BUTTON_VISIBLE);
    button_spacing += buttons[buttons.size() - 1].w + 30;
    ...
    button_sound = 5 - 1;
    y_position = 70;
    create_button(button_spacing, y_position, "", msg, &button_event_survival_time_long,
                  &sound_system.button_mouse_over[button_sound], &sound_system.button_event_fire[button_sound],
                  BUTTON_VISIBLE);
    button_spacing += buttons[buttons.size() - 1].w + 30;
    ...
    button_sound = 7 - 1;
    y_position = 70;
    create_button(button_spacing, y_position, "", msg, &button_event_survival_time_very_long,
                  &sound_system.button_mouse_over[button_sound], &sound_system.button_event_fire[button_sound],
                  BUTTON_VISIBLE);

    msg = "Back to Game Mode Select";
    button_sound = 12 - 1;
    create_button(16, h - back_button_space, "", msg, &button_event_open_window_select_game_mode,
                  &sound_system.button_mouse_over[button_sound], &sound_system.button_event_fire[button_sound],
                  BUTTON_VISIBLE);
    ...
    msg = "Start";
    button_sound = 10 - 1;
    create_button(w - (msg.length() * font.spacing_x) - 16, h - back_button_space, "", msg,
                  &button_event_start_game_survival, &sound_system.button_mouse_over[button_sound],
        ...
        switch (event.type) {
            ...
            case SDL_MOUSEBUTTONUP:
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (player.mouse_allowed()) {
                        for (int i = level_list_display_position, n = 0; i < level_list_display_position + 18;
                             i++, n++) {
                            // If this is a valid level.
                            if (i < levels.size()) {
                                if (collision_check(mouse_x, mouse_y, 1, 1, 0,
                                                    70 + font.spacing_y * 2 + n * font.spacing_y, w, font.spacing_y)) {
...
void Window_Setup_Survival::render () {
    if (on) {
        ...
        // Render the background image.
            ...
            render_sprite(x + 5, y + 5, *background_image, &quickie, 0.25, bg_w / background_image->w,
                          bg_h / background_image->h);
        }
        ...
        for (int i = level_list_display_position, n = 0; i < level_list_display_position + 18; i++, n++) {
            ...
            if (i < levels.size()) {
                ...
                if (collision_check(mouse_x, mouse_y, 1, 1, 0, 70 + font.spacing_y * 2 + n * font.spacing_y, w,
                                    font.spacing_y)) {
                    moused_over = true;
                }

                if (selected && !moused_over) {
                    render_rectangle(x + 20, 70 + font.spacing_y * 2 + n * font.spacing_y, w - 40, font.spacing_y, 1.0,
                                     return_gui_color(holiday, 0));
                    render_rectangle(x + 21, 1 + 70 + font.spacing_y * 2 + n * font.spacing_y, w - 40 - 2,
                                     font.spacing_y - 2, 1.0, return_gui_color(holiday, 4));
                } else if (!selected && moused_over) {
                    render_rectangle(x + 20, 70 + font.spacing_y * 2 + n * font.spacing_y, w - 40, font.spacing_y, 1.0,
                                     return_gui_color(holiday, 0));
                    render_rectangle(x + 21, 1 + 70 + font.spacing_y * 2 + n * font.spacing_y, w - 40 - 2,
                                     font.spacing_y - 2, 1.0, return_gui_color(holiday, 1));
                } else if (selected && moused_over) {
                    render_rectangle(x + 20, 70 + font.spacing_y * 2 + n * font.spacing_y, w - 40, font.spacing_y, 1.0,
                                     return_gui_color(holiday, 0));
                    render_rectangle(x + 21, 1 + 70 + font.spacing_y * 2 + n * font.spacing_y, w - 40 - 2,
                                     font.spacing_y - 2, 1.0, return_gui_color(holiday, 2));
                }

                font.show(x + (w - levels[i].length() * font.spacing_x) / 2 + 2,
                          70 + font.spacing_y * 2 + n * font.spacing_y + 2, levels[i], COLOR_BLACK);
                font.show(x + (w - levels[i].length() * font.spacing_x) / 2,
                          70 + font.spacing_y * 2 + n * font.spacing_y, levels[i], return_gui_color(holiday, 3));
            }
        }
