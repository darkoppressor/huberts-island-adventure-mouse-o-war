...
Moving_Platform::Moving_Platform (double get_move_speed, double get_start_x, double get_start_y, double get_end_x,
                                  double get_end_y, vector<moving_platform_waypoint> get_waypoints, bool get_active,
                                  bool get_round_trip, bool get_movement_type, short get_identifier) {
    // Variables:
        ...
        if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
            ...
            if (vector_npcs[i].exists && !vector_npcs[i].counts_as_trap) {
                ...
                if ((move_state == UP || move_state == LEFT_UP || move_state == RIGHT_UP) &&
                    vector_npcs[i].air_velocity >= 0) {
                    // Check to see if there is a y-axis collision.
                    // We are checking only the bottom row of the pixels for the npc against only the top row of the
                    // pixels for the platform.
                    if (collision_check(x, y, w, h / 12, vector_npcs[i].x,
                                        vector_npcs[i].y + vector_npcs[i].h - (vector_npcs[i].h / 12), vector_npcs[i].w,
                                        vector_npcs[i].h / 12)) {
                        // We set the npc's y value to the y value of this platform - the npc's height, so the npc will
                        // now be right on top of this platform.
                ...
                if ((move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN) &&
                    vector_npcs[i].air_velocity >= 0 && !vector_npcs[i].SWIMMING) {
                    // Check to see if there is a y-axis collision.
                    // We are checking only the bottom row of the pixels for the npc against only the top row of the
                    // pixels for the platform.
                    if (collision_check(x, y, w, h / 12, vector_npcs[i].x,
                                        2 + vector_npcs[i].y + vector_npcs[i].h - (vector_npcs[i].h / 12),
                                        vector_npcs[i].w, 2 + vector_npcs[i].h / 12)) {
                        // We set the npc's y value to the y value of this platform - the npc's height, so the npc will
                        // now be right on top of this platform.
                        vector_npcs[i].y = y - vector_npcs[i].h;
                        // If the npc was in the air, they will no longer be.
    ...
    if (fabs(player.x - x) < PROCESS_RANGE && fabs(player.y - y) < PROCESS_RANGE) {
        if (!player.cheat_noclip) {
            ...
            if ((move_state == UP || move_state == LEFT_UP || move_state == RIGHT_UP) && player.air_velocity >= 0) {
                // Check to see if there is a y-axis collision.
                // We are checking only the bottom row of the pixels for the player against only the top row of the
                // pixels for the platform.
                if (collision_check(x, y, w, h / 12, player.x, player.y + player.h - (player.h / 12), player.w,
                                    player.h / 12)) {
                    // We set the player's y value to the y value of this platform - the player's height, so the player
                    // will now be right on top of this platform.
                    player.y = y - player.h;
            ...
            if ((move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN) &&
                player.air_velocity >= 0 && !player.SWIMMING) {
                // Check to see if there is a y-axis collision.
                // We are checking only the bottom row of the pixels for the player against only the top row of the
                // pixels for the platform.
                if (collision_check(x, y, w, h / 12, player.x, 2 + player.y + player.h - (player.h / 12), player.w,
                                    2 + player.h / 12)) {
                    // We set the player's y value to the y value of this platform - the player's height, so the player
                    // will now be right on top of this platform.
                    player.y = y - player.h;
    ...
    for (int i = 0; i < mp_players.size(); i++) {
        if (fabs(mp_players[i].x - x) < PROCESS_RANGE && fabs(mp_players[i].y - y) < PROCESS_RANGE) {
            ...
            if ((move_state == UP || move_state == LEFT_UP || move_state == RIGHT_UP) &&
                ...
                // We are checking only the bottom row of the pixels for the player against only the top row of the
                // pixels for the platform.
                if (collision_check(x, y, w, h / 12, mp_players[i].x,
                                    mp_players[i].y + mp_players[i].h - (mp_players[i].h / 12), mp_players[i].w,
                                    mp_players[i].h / 12)) {
                    // We set the player's y value to the y value of this platform - the player's height, so the player
                    // will now be right on top of this platform.
            ...
            // If the platform moves down with the player on top of it.
            if ((move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN) &&
                mp_players[i].air_velocity >= 0 && !mp_players[i].SWIMMING) {
                // Check to see if there is a y-axis collision.
                // We are checking only the bottom row of the pixels for the player against only the top row of the
                // pixels for the platform.
                if (collision_check(x, y, w, h / 12, mp_players[i].x,
                                    2 + mp_players[i].y + mp_players[i].h - (mp_players[i].h / 12), mp_players[i].w,
                                    2 + mp_players[i].h / 12)) {
                    // We set the player's y value to the y value of this platform - the player's height, so the player
                    // will now be right on top of this platform.
                    mp_players[i].y = y - mp_players[i].h;
...
void Moving_Platform::render (bool mirrored) {
    ...
    if (x >= player.camera_x - w && x <= player.camera_x + player.camera_w && y >= player.camera_y - h &&
        ...
        if (move_state != 0) {
            if (facing == LEFT) {
                render_sprite((int) ((int) render_x - 5 - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_moving_platform,
                              &sprites_moving_platform_left[frame]);
            } else if (facing == RIGHT) {
                render_sprite((int) ((int) render_x - 5 - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_moving_platform,
                              &sprites_moving_platform_right[frame]);
            } else if (facing == UP) {
                render_sprite((int) ((int) render_x - 5 - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_moving_platform,
                              &sprites_moving_platform_up[frame]);
            } else if (facing == DOWN) {
                render_sprite((int) ((int) render_x - 5 - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_moving_platform,
                              &sprites_moving_platform_down[frame]);
            }
        }
        // If the platform is not moving.
            ...
            render_sprite((int) ((int) render_x - 5 - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_moving_platform,
                          &sprites_moving_platform_idle[frame_idle]);
        }
    }
}
