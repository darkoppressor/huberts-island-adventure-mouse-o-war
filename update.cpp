...
void input () {
    ...
    if (!player.pause && player.game_in_progress) {
        ...
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (fabs(vector_npcs[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_npcs[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
...
void finger_down (const SDL_Event& event_storage) {
    vector<SDL_Scancode> touch_buttons = touch_controller.check_for_button_press(event_storage.tfinger.x,
                                                                                 event_storage.tfinger.y);
...
void movement () {
    ...
    if (!player.pause && player.game_in_progress) {
        ...
        for (int i = 0; i < vector_traps.size(); i++) {
            if (fabs(vector_traps[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_traps[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
                vector_traps[i].move();
            }
        }
        ...
        for (int i = 0; i < vector_shots.size(); i++) {
            if (fabs(vector_shots[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_shots[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
                vector_shots[i].move();
            }
        }
        ...
        for (int i = 0; i < vector_items.size(); i++) {
            if (fabs(vector_items[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_items[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
                vector_items[i].move();
            }
        }
...
void events () {
    if (player.game_mode == GAME_MODE_SP_ADVENTURE) {
        ...
        if (level.timer_survival_escape_counter.is_started() &&
            level.timer_survival_escape_counter.get_ticks() >= 1000) {
        ...
        if (level.timer_survival_spawn_items.is_started() &&
            level.timer_survival_spawn_items.get_ticks() >= 1000 * level.survival_spawn_items_time()) {
        ...
        if (level.timer_survival_spawn_npcs.is_started() &&
            level.timer_survival_spawn_npcs.get_ticks() >= 1000 * level.survival_spawn_npcs_time()) {
    ...
    if (!player.pause && player.game_in_progress) {
        ...
        for (int i = 0; i < vector_triggers.size(); i++) {
            if (fabs(vector_triggers[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_triggers[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
...
void animation () {
    ...
    if (!player.pause && player.game_in_progress) {
        ...
        for (int i = 0; i < vector_moving_platforms.size(); i++) {
            if (fabs(vector_moving_platforms[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_moving_platforms[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
        ...
        for (int i = 0; i < vector_doors.size(); i++) {
            if (fabs(vector_doors[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_doors[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
        ...
        for (int i = 0; i < vector_springs.size(); i++) {
            if (fabs(vector_springs[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_springs[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
        ...
        for (int i = 0; i < vector_boosters.size(); i++) {
            if (fabs(vector_boosters[i].x - player.cam_focused_x()) < PROCESS_RANGE &&
                fabs(vector_boosters[i].y - player.cam_focused_y()) < PROCESS_RANGE) {
    ...
    if (new_shots.size() > 0) {
        for (int i = 0; i < new_shots.size(); i++) {
            vector_shots.push_back(Shot(new_shots[i].x, new_shots[i].y, new_shots[i].angle, new_shots[i].type,
                                        new_shots[i].counts_as_trap));
        }
    }
...
void render (int frame_rate, double ms_per_frame, int logic_frame_rate) {
    ...
    if (player.game_in_progress) {
            ...
            for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
                ...
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if (!level.tile_array[int_x][int_y].foreground &&
                        level.tile_array[int_x][int_y].special == TILE_SPECIAL_MIRROR) {
        ...
        if (player.fadeout) {
            render_rectangle(0, 0, main_window.SCREEN_WIDTH, main_window.SCREEN_HEIGHT, player.fade_opacity,
                             COLOR_BLACK);
        }
        ...
        if (player.DYING) {
            if (player.game_mode == GAME_MODE_SP_ADVENTURE) {
                ...
                ss.str("");
                ss << "Death #";
                ss << player.stat_deaths_enemies + player.stat_deaths_traps + player.stat_deaths_drowning;
                msg = ss.str();
                font_large.show((main_window.SCREEN_WIDTH - msg.length() * font_large.spacing_x * 4.0) / 2.0 + 2,
                                (main_window.SCREEN_HEIGHT - font_large.spacing_y * 4.0) / 2.0 + 2, msg, COLOR_BLACK,
                                1.0, 4.0);
                font_large.show((main_window.SCREEN_WIDTH - msg.length() * font_large.spacing_x * 4.0) / 2.0,
                                (main_window.SCREEN_HEIGHT - font_large.spacing_y * 4.0) / 2.0, msg,
                                return_gui_color(holiday, 3), 1.0, 4.0);
            }
        }

        // If the game is paused.
        if (!player.hide_gui && player.pause && window_manager.which_window_open() == -1 &&
            !level.return_title_pause()) {
            render_rectangle((main_window.SCREEN_WIDTH - 160) / 2, (main_window.SCREEN_HEIGHT - 85) / 2, 160, 85, 1.0,
                             return_gui_color(holiday, 0));
            render_rectangle((main_window.SCREEN_WIDTH - 160) / 2 + 2, (main_window.SCREEN_HEIGHT - 85) / 2 + 2,
                             160 - 4, 85 - 4, 1.0, return_gui_color(holiday, 1));
            font.show((main_window.SCREEN_WIDTH - 71) / 2 + 2, (main_window.SCREEN_HEIGHT - 16) / 2 + 2, "Paused",
                      COLOR_BLACK);
            font.show((main_window.SCREEN_WIDTH - 71) / 2, (main_window.SCREEN_HEIGHT - 16) / 2, "Paused",
                      return_gui_color(holiday, 3));
        }

        if (!player.hide_gui && !level.return_title_pause() &&
            ((player.pause && window_manager.which_window_open() == -1) ||
             (window_manager.which_window_open() != -1 &&
              window_manager.which_window_open() != WHICH_WINDOW_MESSAGE))) {
            player.render_menu_pretties();
        }
    }

    if (!player.hide_gui) {
        if ((player.game_in_progress && !level.return_title_pause() &&
             ((player.pause && window_manager.which_window_open() == -1) ||
              (window_manager.which_window_open() != -1 &&
               window_manager.which_window_open() != WHICH_WINDOW_MESSAGE))) ||
    ...
    if (!player.hide_gui && player.game_in_progress) {
        if (player.on_worldmap()) {
            if (player.option_hints && tooltip_levelname.on) {
                for (int i = player.worldmap_level_rect_color_numbers.size() - 1; i >= 0; i--) {
                    render_rectangle_empty(0, 0, main_window.SCREEN_WIDTH, main_window.SCREEN_HEIGHT, 1.0,
                                           return_gui_color(holiday, player.worldmap_level_rect_color_numbers[i]),
                                           12.0 + i * 12.0);
                }
            }
        }
        ...
        if (!player.game_mode_is_multiplayer()) {
            render_sprite(15 - 5 + 2, 10 + 2, image.sprite_sheet_items, &sprites_item_ammo[0], 1.0, 1.0, 1.0, 0.0,
                          COLOR_BLACK);
            ...
            for (int i = -1; i < (int) mp_players.size(); i++) {
                ...
                render_sprite(215 - 5 + ((i + 1) * 100) + 2, 10 + 2, image.sprite_sheet_items, &sprites_item_ammo[0],
                              1.0, 1.0, 1.0, 0.0, COLOR_BLACK);
                render_sprite(215 - 5 + ((i + 1) * 100), 10, image.sprite_sheet_items, &sprites_item_ammo[0], 1.0, 1.0,
                              1.0);
                ss.clear();
                ss.str("");
                ss << "x ";
        ...
        if (!player.on_worldmap()) {
            if (!player.game_mode_is_multiplayer()) {
                ...
                if (player.shot_homing) {
                    ...
                    if (player.current_shot == SHOT_PLAYER) {
                        render_sprite(352 + 2, 18 + 2, image.shot_types, &sprites_shot_types[0], 1.0, 1.0, 1.0, 0.0,
                                      COLOR_BLACK);
                        render_sprite(352, 18, image.shot_types, &sprites_shot_types[0]);
                    } else if (player.current_shot == SHOT_PLAYER_SPLODE) {
                        render_sprite(352 + 2, 18 + 2, image.shot_types, &sprites_shot_types[1], 1.0, 1.0, 1.0, 0.0,
                                      COLOR_BLACK);
                        render_sprite(352, 18, image.shot_types, &sprites_shot_types[1]);
                    } else if (player.current_shot == SHOT_PLAYER_HOMING) {
                        render_sprite(352 + 2, 18 + 2, image.shot_types, &sprites_shot_types[2], 1.0, 1.0, 1.0, 0.0,
                                      COLOR_BLACK);
                ...
                render_texture(352 + 48 + ITEM_W + 7 + player_box_offset + 2, 18 + 2,
                               *player.return_character_box_image(), 1.0, COLOR_BLACK);
                ...
                if (!player.pause && player.all_humans_dead()) {
                    ...
                    tooltip_hint.setup(tooltip_text, (player.camera_w - tooltip_text.length() * font.spacing_x) / 2.0,
                                       84);
                }
            }

            if (player.game_mode == GAME_MODE_SP_ADVENTURE || player.game_mode == GAME_MODE_MP_ADVENTURE) {
                render_sprite(650 + 2, 30 + 2, image.sprite_sheet_items, &sprites_item_leaf[0], 1.0, 1.0, 1.0, 0.0,
                              COLOR_BLACK);
                render_sprite(650, 30, image.sprite_sheet_items, &sprites_item_leaf[0]);
                ss.clear();
                ss.str("");
                ss << "x ";
                ss << player.current_level_leaves;
                ss << "/";
                ss << level.leaves;
                msg = ss.str();
                ...
                render_sprite(650 + 2, 50 + 2, image.sprite_sheet_items, &sprites_item_cheese[0], 1.0, 0.25, 0.25, 0.0,
                              COLOR_BLACK);
                render_sprite(650, 50, image.sprite_sheet_items, &sprites_item_cheese[0], 1.0, 0.25, 0.25);
                ss.clear();
                ss.str("");
                ss << "x ";
                ss << player.current_level_cheese;
                ss << "/";
                ss << level.cheese;
                msg = ss.str();
                ...
                font.show((main_window.SCREEN_WIDTH - time_string.length() * font.spacing_x) / 2.0 + 2, 64 + 2,
                          time_string, COLOR_BLACK);
                font.show((main_window.SCREEN_WIDTH - time_string.length() * font.spacing_x) / 2.0, 64, time_string,
                          return_gui_color(holiday, 3));
            } else if (player.game_mode == GAME_MODE_MP_SURVIVAL) {
                string time_string = get_time_string_short(level.survival_seconds_played);
            ...
            if (player.boss_index != -1) {
                double health_percentage = (double) vector_npcs[player.boss_index].health /
                                           (double) vector_npcs[player.boss_index].health_max;
                double health_bar_width = 240.0 * health_percentage;
                double health_max_bar_width = 240.0 * 1.0;
                string boss_text = player.num_to_string(vector_npcs[player.boss_index].health) + "/" +
                                   player.num_to_string(vector_npcs[player.boss_index].health_max);

                render_rectangle(main_window.SCREEN_WIDTH / 2.0 - 120 - 2, main_window.SCREEN_HEIGHT - 75 - 2,
                                 health_max_bar_width + 4, 30 + 4, 1.0, COLOR_BLACK);
                render_rectangle(main_window.SCREEN_WIDTH / 2.0 - 120, main_window.SCREEN_HEIGHT - 75, health_bar_width,
                                 30, 1.0, COLOR_RED);

                font.show((main_window.SCREEN_WIDTH - boss_text.length() * font.spacing_x) / 2.0 + 2,
                          main_window.SCREEN_HEIGHT - 68 + 2, boss_text, COLOR_BLACK);
                font.show((main_window.SCREEN_WIDTH - boss_text.length() * font.spacing_x) / 2.0,
                          main_window.SCREEN_HEIGHT - 68, boss_text, COLOR_WHITE);
            }
            ...
            if (!player.game_mode_is_multiplayer()) {
                ...
                if (player.oxygen < player.oxygen_max_capacity) {
                    ...
                    render_rectangle(main_window.SCREEN_WIDTH / 2.0 - 60 - 2, 75 - 2, oxygen_max_bar_width + 4, 30 + 4,
                                     1.0, COLOR_BLACK);
                    render_rectangle(main_window.SCREEN_WIDTH / 2.0 - 60, 75, oxygen_bar_width, 30, 1.0,
                                     COLOR_BLUE_OCEAN);
                }
            } else {
                for (int i = -1; i < (int) mp_players.size(); i++) {
                    ...
                    if (!PLAYER_DYING && !PLAYER_BUBBLE_MODE && PLAYER_OXYGEN < PLAYER_OXYGEN_MAX_CAPACITY) {
                        ...
                        render_rectangle(220 + ((i + 1) * 100) - 2, 96 - 2, oxygen_max_bar_width + 4, 15 + 4, 1.0,
                                         COLOR_BLACK);
        ...
        render_sprite(650 + 2, 10 + 2, image.sprite_sheet_items, &sprites_item_candy[0], 1.0, 1.0, 1.0, 0.0,
                      COLOR_BLACK);
    ...
    /**ss.clear();ss.str("");ss<<"Number of joysticks: ";ss<<joystick.size();ss<<"\xA";msg=ss.str();
       for(int i=0;i<joystick.size();i++){
        if(SDL_JoystickGetAttached(joystick[i].joy)){
            ss.clear();ss.str("");ss<<"Joystick "<<i<<"'s name:
               ";ss<<SDL_JoystickName(joystick[i].joy);ss<<"\xA";msg+=ss.str();
            ss.clear();ss.str("");ss<<"Number of axes:
               ";ss<<SDL_JoystickNumAxes(joystick[i].joy);ss<<"\xA";msg+=ss.str();
            ss.clear();ss.str("");ss<<"Number of buttons:
               ";ss<<SDL_JoystickNumButtons(joystick[i].joy);ss<<"\xA";msg+=ss.str();
            ss.clear();ss.str("");ss<<"Number of balls:
               ";ss<<SDL_JoystickNumBalls(joystick[i].joy);ss<<"\xA";msg+=ss.str();
            ss.clear();ss.str("");ss<<"Number of hats:
               ";ss<<SDL_JoystickNumHats(joystick[i].joy);ss<<"\xA\xA";msg+=ss.str();
        }
       }
    ...
    if (!player.touch_controls && !player.option_hardware_cursor &&
        (window_manager.which_window_open() != -1 || player.cam_state != CAM_STICKY)) {
        int mouse_x = 0;
        int mouse_y = 0;
...
void render_loading_screen (double percentage, int level_loading) {
    if (main_window.is_initialized()) {
        ...
        render_rectangle(main_window.SCREEN_WIDTH / 2.0 - 120 - 2, main_window.SCREEN_HEIGHT - 75 - 2,
                         max_bar_width + 4, 30 + 4, 1.0, COLOR_BLACK);
        render_rectangle(main_window.SCREEN_WIDTH / 2.0 - 120, main_window.SCREEN_HEIGHT - 75, bar_width, 30, 1.0,
                         return_gui_color(holiday, 5));

        ss.clear();
        ss.str("");
        ss << (int) (percentage * 100.0);
        ss << "%";
        msg = ss.str();
        font.show(main_window.SCREEN_WIDTH / 2.0 - 120 + (max_bar_width - msg.length() * font.spacing_x) / 2.0 + 2,
                  main_window.SCREEN_HEIGHT - 75 + font.spacing_y / 4.0 + 2, msg, COLOR_BLACK);
        font.show(main_window.SCREEN_WIDTH / 2.0 - 120 + (max_bar_width - msg.length() * font.spacing_x) / 2.0,
                  main_window.SCREEN_HEIGHT - 75 + font.spacing_y / 4.0, msg, return_gui_color(holiday, 6));
