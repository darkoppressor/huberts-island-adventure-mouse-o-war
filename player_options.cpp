...
bool Player::check_key (Input_Data checked_key) {
    ...
    for (int i = 0; i < keys.size(); i++) {
        // If the current command has the same input type as the checked command, and if the command is not the one
        // being set.
        // We do the second check so if a command is set to enter, and we are setting that command, enter will not count
        // as taken.
        if (keys[i].type == checked_key.type && (i != command_to_set || -1 != command_to_set_player)) {
            switch (checked_key.type) {
                ...
                case INPUT_TYPE_JOYSTICK_BUTTON:
                    ...
                    if (keys[i].which_joystick == checked_key.which_joystick &&
                        keys[i].joy_button == checked_key.joy_button) {
                        input_already_bound = i;
                        input_already_bound_player = -1;
                ...
                case INPUT_TYPE_JOYSTICK_AXIS:
                    ...
                    if (keys[i].which_joystick == checked_key.which_joystick &&
                        keys[i].joy_axis == checked_key.joy_axis &&
                        keys[i].joy_axis_direction == checked_key.joy_axis_direction) {
                        input_already_bound = i;
                        input_already_bound_player = -1;
                ...
                case INPUT_TYPE_JOYSTICK_HAT:
                    ...
                    if (keys[i].which_joystick == checked_key.which_joystick &&
                        keys[i].joy_hat == checked_key.joy_hat &&
                        keys[i].joy_hat_direction == checked_key.joy_hat_direction) {
                        input_already_bound = i;
                        input_already_bound_player = -1;
                ...
                case INPUT_TYPE_JOYSTICK_BALL:
                    ...
                    if (keys[i].which_joystick == checked_key.which_joystick &&
                        keys[i].joy_ball == checked_key.joy_ball &&
                        keys[i].joy_ball_direction == checked_key.joy_ball_direction) {
                        input_already_bound = i;
                        input_already_bound_player = -1;
    ...
    for (int i = 0; i < mp_keys.size(); i++) {
        for (int n = 0; n < mp_keys[i].size(); n++) {
            // If the current command has the same input type as the checked command, and if the command is not the one
            // being set.
            // We do the second check so if a command is set to enter, and we are setting that command, enter will not
            // count as taken.
            if (mp_keys[i][n].type == checked_key.type && (n != command_to_set || i != command_to_set_player)) {
                switch (checked_key.type) {
                    ...
                    case INPUT_TYPE_JOYSTICK_BUTTON:
                        ...
                        if (mp_keys[i][n].which_joystick == checked_key.which_joystick &&
                            mp_keys[i][n].joy_button == checked_key.joy_button) {
                            input_already_bound = n;
                            input_already_bound_player = i;
                    ...
                    case INPUT_TYPE_JOYSTICK_AXIS:
                        ...
                        if (mp_keys[i][n].which_joystick == checked_key.which_joystick &&
                            mp_keys[i][n].joy_axis == checked_key.joy_axis &&
                            mp_keys[i][n].joy_axis_direction == checked_key.joy_axis_direction) {
                            input_already_bound = n;
                            input_already_bound_player = i;
                    ...
                    case INPUT_TYPE_JOYSTICK_HAT:
                        ...
                        if (mp_keys[i][n].which_joystick == checked_key.which_joystick &&
                            mp_keys[i][n].joy_hat == checked_key.joy_hat &&
                            mp_keys[i][n].joy_hat_direction == checked_key.joy_hat_direction) {
                            input_already_bound = n;
                            input_already_bound_player = i;
                    ...
                    case INPUT_TYPE_JOYSTICK_BALL:
                        ...
                        if (mp_keys[i][n].which_joystick == checked_key.which_joystick &&
                            mp_keys[i][n].joy_ball == checked_key.joy_ball &&
                            mp_keys[i][n].joy_ball_direction == checked_key.joy_ball_direction) {
                            input_already_bound = n;
                            input_already_bound_player = i;
    ...
    if (command_to_set != -1) {
        ...
        switch (event.type) {
                ...
                if (event.jaxis.value < JOYSTICK_NEUTRAL_NEGATIVE) {
                    ...
                    if (!check_key(key_to_check)) {
                        if (command_to_set_player == -1) {
                            ...
                            mp_keys[command_to_set_player][command_to_set].joy_axis_direction =
                                key_to_check.joy_axis_direction;
                        }
                ...
                if (event.jhat.value != SDL_HAT_CENTERED) {
                    ...
                    if (!check_key(key_to_check)) {
                        if (command_to_set_player == -1) {
                            ...
                            mp_keys[command_to_set_player][command_to_set].joy_hat_direction =
                                key_to_check.joy_hat_direction;
                        }
            ...
            case SDL_JOYBALLMOTION:
                ...
                ///I am assuming that on the x-axis, negative relative motion means left and positive relative motion
                // means right.
                ...
                if (!check_key(key_to_check)) {
                    if (command_to_set_player == -1) {
                        ...
                        mp_keys[command_to_set_player][command_to_set].joy_ball_direction =
                            key_to_check.joy_ball_direction;
                    }
