    ...
    if (exists) {
        ...
        if (!special_attack_in_progress) {
            ...
            if (wander && !ai_player_followable(player_index) && (!flee_player || !ai_player_fleeable(player_index))) {
                ...
                // If the wander counter meets the wander frequency, or if wander_notice_edges is true and an edge has
                // been encountered,
                // or if wander_notice_walls is true and a wall has been encountered,
                // change the wander direction.
                if (wander_counter >= wander_frequency || (wander_notice_edges && ai_notice_edge()) ||
                    (wander_notice_walls && ai_notice_wall())) {
                    wander_counter = 0;
            ...
            if (special_attack_range_x != -1 && special_attack_counter == 0 &&
                ...
                (!special_attack_only_when_player_facing ||
                 (special_attack_only_when_player_facing && !ai_player_back_turned(player_index))) &&
                (!special_attack_only_when_facing_player ||
                 (special_attack_only_when_facing_player && ai_facing_player(player_index)))) {
                ...
                // If the npc can use its special attack on sight.
                    ...
                    if (special_attack_below_only && collision_check(x - TILE_SIZE * special_attack_range_x, y + h,
                                                                     w + (TILE_SIZE * special_attack_range_x) * 2,
                                                                     TILE_SIZE * special_attack_range_y, PLAYER_X,
                    ...
                    } else if (special_attack_above_only && collision_check(x - TILE_SIZE * special_attack_range_x,
                                                                            y - TILE_SIZE * special_attack_range_y,
                                                                            w + (TILE_SIZE * special_attack_range_x) *
                                                                            2, TILE_SIZE * special_attack_range_y,
                                                                            PLAYER_X, PLAYER_Y, PLAYER_WIDTH,
                                                                            PLAYER_HEIGHT)) {
                        ai_initiate_special_attack(player_index);
                    } else if (!special_attack_below_only && !special_attack_above_only &&
                               collision_check(x - TILE_SIZE * special_attack_range_x,
                                               y - TILE_SIZE * special_attack_range_y,
                                               w + (TILE_SIZE * special_attack_range_x) * 2,
                                               h + (TILE_SIZE * special_attack_range_y) * 2, PLAYER_X, PLAYER_Y,
...
void Npc::ai_attempt_use () {
    if (use_counter == 0 && use && (!use_only_while_alarm_off || (use_only_while_alarm_off && !level.alarm))) {
...
void Npc::ai_initiate_special_attack (int player_index) {
    ...
    double angle = ai_get_target_direction(special_attack_target_x, special_attack_target_y, PLAYER_WIDTH,
                                           PLAYER_HEIGHT);
...
bool Npc::ai_player_in_range (short range_x, short range_y, int player_index) {
    ...
    // If the player is in range.
    if (collision_check(x - TILE_SIZE * range_x, y - TILE_SIZE * range_y, w + (TILE_SIZE * range_x) * 2,
                        h + (TILE_SIZE * range_y) * 2, PLAYER_X, PLAYER_Y, PLAYER_WIDTH, PLAYER_HEIGHT)) {
        return true;
    }
...
bool Npc::ai_player_back_turned (int player_index) {
    ...
    if (!PLAYER_BUBBLE_MODE) {
        ...
        if (((!player_back_turned_when_above && PLAYER_X < x + w) ||
             (player_back_turned_when_above && PLAYER_X + PLAYER_WIDTH < x)) &&
            (PLAYER_FACING == LEFT || PLAYER_FACING == LEFT_UP || PLAYER_FACING == LEFT_DOWN)) {
            return true;
        }
        // If the player is to the right of the npc and is facing right.
        else if (((!player_back_turned_when_above && PLAYER_X + PLAYER_WIDTH > x) ||
                  (player_back_turned_when_above && PLAYER_X > x + w)) &&
                 (PLAYER_FACING == RIGHT || PLAYER_FACING == RIGHT_UP || PLAYER_FACING == RIGHT_DOWN)) {
            return true;
        }
    } else {
        return true;
...
bool Npc::ai_player_followable (int player_index) {
    ...
    if (!PLAYER_DYING && !PLAYER_BUBBLE_MODE && ai_player_in_range(follow_player_range_x, follow_player_range_y,
                                                                   player_index)) {
        if (!follow_player_only_when_angry || (follow_player_only_when_angry && angry)) {
            ...
            if ((follow_player || (follow_player_during_alarm && level.alarm)) &&
                ((follow_player_during_alarm && level.alarm) ||
                 ((!follow_player_only_in_water || (follow_player_only_in_water && PLAYER_SWIMMING)) &&
                  ...
                  (!follow_player_only_when_back_turned ||
                   (follow_player_only_when_back_turned && ai_player_back_turned(player_index)))))) {
                return true;
            }
        }
    }
...
void Npc::ai_move_to_target (double target_x, double target_y, double target_w, double target_h) {
    ...
    if (!collision_check(this_width_box_x, this_width_box_y, this_width_box_w, this_width_box_h, target_width_box_x,
                         target_width_box_y, target_width_box_w, target_width_box_h)) {
        if (x + w / 2.0 > target_x + target_w / 2.0) {
            ai_keystates[AIK_LEFT] = true;
        } else if (x + w / 2.0 < target_x + target_w / 2.0) {
            ai_keystates[AIK_RIGHT] = true;
    ...
    if (!collision_check(this_height_box_x, this_height_box_y, this_height_box_w, this_height_box_h,
                         target_height_box_x, target_height_box_y, target_height_box_w, target_height_box_h)) {
        if (y + h / 2.0 > target_y + target_h / 2.0) {
            ai_keystates[AIK_UP] = true;
        } else if (y + h / 2.0 < target_y + target_h / 2.0) {
            ai_keystates[AIK_DOWN] = true;
...
double Npc::ai_get_target_direction (double target_x, double target_y, double target_w, double target_h) {
    ...
    y_component = sqrt(((rotation_target_x - x2) * (rotation_target_x - x2)) +
                       ((rotation_target_y - y2) * (rotation_target_y - y2)));
            ...
            for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
                ...
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if (level.tile_array[int_x][int_y].solidity != TILE_SOLIDITY_SOLID &&
                        ...
                        if (collision_check(x - 2, y + h, w, TILE_SIZE, level.tile_array[int_x][int_y].x,
                                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                            if (level.tile_array[int_x][int_y].x<x && level.tile_array[int_x][int_y].y> y &&
    ...
    if (FLYING) {
                ...
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID ||
                        ...
                        (level.tile_array[int_x][int_y].special == TILE_SPECIAL_CLIMBABLE_TOP &&
                         (move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN)) ||
                        (!notice_wall_ignore_cloud && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_CLOUD &&
                         (move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN))) {
        ...
        for (int i = 0; i < vector_moving_platforms.size(); i++) {
            if (fabs(vector_moving_platforms[i].x - x) < PROCESS_RANGE &&
                ...
                if (collision_check(x, y + 2, w, h, vector_moving_platforms[i].x, vector_moving_platforms[i].y,
                                    vector_moving_platforms[i].w, vector_moving_platforms[i].h)) {
                    if (vector_moving_platforms[i].y > y &&
                        (move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN)) {
                        return true;
                    }
                }
        ...
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (this != &vector_npcs[i] && vector_npcs[i].exists) {
                if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                    ...
                    if (collision_check(x - 2, y, w, h, vector_npcs[i].x, vector_npcs[i].y, vector_npcs[i].w,
                                        vector_npcs[i].h)) {
                        if (vector_npcs[i].x < x &&
                            (move_state == LEFT || move_state == LEFT_UP || move_state == LEFT_DOWN)) {
                            return true;
                        }
                    }
                    ...
                    // If the object is just below the npc and the npc is wandering down.
                    if (collision_check(x, y + 2, w, h, vector_npcs[i].x, vector_npcs[i].y, vector_npcs[i].w,
                                        vector_npcs[i].h)) {
                        if (vector_npcs[i].y > y &&
                            (move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN)) {
                            return true;
                        }
                    }
        ...
        for (int i = 0; i < vector_traps.size(); i++) {
            if (fabs(vector_traps[i].x - x) < PROCESS_RANGE && fabs(vector_traps[i].y - y) < PROCESS_RANGE) {
                // If the object is just to the npc's left and the npc is wandering left.
                if (collision_check(x - 2, y, w, h, vector_traps[i].x, vector_traps[i].y, vector_traps[i].w,
                                    vector_traps[i].h)) {
