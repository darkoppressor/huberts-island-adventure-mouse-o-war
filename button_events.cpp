...
void button_event_open_window_map_other_level (Window* parent_window, int special_data) {
    ...
    if (special_data == player.current_level) {
        window_map[0].set_camera_coordinates(player.x / 2.0 + (player.w / 2.0 - window_map[0].map_camera_w) / 2.0,
                                             player.y / 2.0 + (player.h / 2.0 - window_map[0].map_camera_h) / 2.0);
    } else {
        Spawn_Coords sc = profile.return_level_spawn(special_data);

        window_map[0].set_camera_coordinates(sc.x / 2.0 + (player.w / 2.0 - window_map[0].map_camera_w) / 2.0,
                                             sc.y / 2.0 + (player.h / 2.0 - window_map[0].map_camera_h) / 2.0);
    }
...
void button_event_buy_game (Window* parent_window, int special_data) {
    string buy_url = "https://cheeseandbacon.org/huberts-island/buy.php";
