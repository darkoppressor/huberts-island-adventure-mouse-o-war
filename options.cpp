...
int options_which_version_series (vector<Version_Series>* version_series, int major, int minor, int micro) {
    for (int i = 0; i < version_series->size(); i++) {
        if (options_version_compare(major, minor, micro, version_series->at(i).major_1, version_series->at(i).minor_1,
                                    version_series->at(i).micro_1) >= 0 && options_version_compare(major, minor, micro,
                                                                                                   version_series->at(
                                                                                                       i).major_2,
                                                                                                   version_series->at(
                                                                                                       i).minor_2,
...
bool options_version_compatible (string name_to_check) {
    ...
    if (options_which_version_series(&version_series, major, minor,
                                     micro) != options_which_version_series(&version_series, current_major,
                                                                            current_minor, current_micro)) {
        string error_message = "Version incompatibility! The profile '" + name_to_check +
                               "' was started with version " + player.num_to_string(major) + "." +
                               player.num_to_string(minor) + "." + player.num_to_string(micro) + ".";

        error_message += " Current version is " + player.num_to_string(current_major) + "." +
                         player.num_to_string(current_minor) + "." + player.num_to_string(current_micro) + ".";
