...
Effect_Firework::Effect_Firework (double get_x, double get_y, int magnitude, short get_color, short shape,
                                  int thickness, double speed_x, double speed_y) {
    x = get_x;
    y = get_y;
    ...
    for (int i = 0; i < amount; i++) {
        ...
        if (fabs(p_vector_x) - magnitude < 10 && fabs(p_vector_y) - magnitude < 10 && random_range(0, 99) < 80) {
            if (random_range(0, 1) == 0) {
                if (p_vector_x > 0.0) {
...
void Effect_Firework::render () {
    if (exists) {
        if (exploding) {
            if (x >= player.camera_x - 1600 && x - 1600 <= player.camera_x + player.camera_w &&
                y >= player.camera_y - 1600 && y - 1600 <= player.camera_y + player.camera_h) {
                for (int i = 0; i < particles.size(); i++) {
                    particles[i].render();
                }
            }
            ...
            if (x >= player.camera_x - render_width && x <= player.camera_x + player.camera_w &&
                y >= player.camera_y - render_height && y <= player.camera_y + player.camera_h) {
                render_rectangle((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                                 render_width, render_height, 1.0, color);
            }
        }
    }
}

Effect_Firework_Particle::Effect_Firework_Particle (double get_x, double get_y, double get_vector_x,
                                                    double get_vector_y, short get_color) {
    x = get_x;
    y = get_y;
...
void Effect_Firework_Particle::render () {
    if (exists) {
        ...
        if (x >= player.camera_x - render_width && x <= player.camera_x + player.camera_w &&
            y >= player.camera_y - render_height && y <= player.camera_y + player.camera_h) {
            render_rectangle((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                             render_width, render_height, opacity, color);
        }
    }
}
