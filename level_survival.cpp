...
int Level::survival_spawn_items_chance () {
    unsigned long minutes_played = survival_seconds_played / 60;
    unsigned long base_chance = (unsigned long) floor(((double) (410.0 /
                                                                 (double) ((level_x / TILE_SIZE) *
                                                                           (level_y / TILE_SIZE))) * 1000.0) * 2.0);
...
int Level::survival_spawn_npcs_chance () {
    ...
    unsigned long base_chance = (unsigned long) floor(((double) (410.0 /
                                                                 (double) ((level_x / TILE_SIZE) *
                                                                           (level_y / TILE_SIZE))) * 1000.0) * 2.0) / 2;
...
void Level::load_level_survival () {
    ...
    if (load.is_opened()) {
        ...
        while (!data_stream.eof()) {
            ...
            if (move_speed != 0) {
                vector_moving_platforms.push_back(Moving_Platform(move_speed, start_point_x, start_point_y, end_point_x,
                                                                  end_point_y, waypoints, active, round_trip,
    ...
    if (load.is_opened()) {
        ...
        while (!data_stream.eof()) {
            ...
            if (type == TRAP_WEATHER_MACHINE || type == TRAP_OPEN_SHOP || type == TRAP_OPEN_UPGRADES ||
                type == TRAP_BOSS_CASTLE || type == TRAP_BOSS_PYRAMID) {
                should_spawn = false;
            }
    ...
    for (int i = 0; i < vector_items.size(); i++) {
        ...
        if (vector_items[i].type != ITEM_SPAWNPOINT) {
            vector_spawners.push_back(Spawner(SPAWN_ITEM, vector_items[i].type, vector_items[i].x, vector_items[i].y,
                                              false, items_stay));
    ...
    for (int i = 0; i < vector_npcs.size(); i++) {
        ...
        if (vector_npcs[i].type == NPC_MONKEY || vector_npcs[i].type == NPC_ARCHER ||
            vector_npcs[i].type == NPC_SCARECROW || vector_npcs[i].type == NPC_EAGLE) {
            disallow_doubles = true;
        }

        if (should_become_spawner) {
            vector_spawners.push_back(Spawner(SPAWN_NPC, vector_npcs[i].type, vector_npcs[i].x, vector_npcs[i].y,
                                              disallow_doubles, false));
