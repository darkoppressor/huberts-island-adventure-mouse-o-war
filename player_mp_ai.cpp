    ...
    if (counter_decision_cooldown_revive == 0) {
        ...
        if (anyone_dead) {
            ...
            for (int i = 0; i < vector_items.size(); i++) {
                if (fabs(vector_items[i].x - x) < PROCESS_RANGE && fabs(vector_items[i].y - y) < PROCESS_RANGE) {
                    if (vector_items[i].exists &&
                        (vector_items[i].type == ITEM_SPAWNPOINT || vector_items[i].type == ITEM_CHECKPOINT ||
            ...
            if (nearest_item != -1) {
                ...
                decision_target.y = (int) (((int) vector_items[nearest_item].y + (int) vector_items[nearest_item].h /
                                            2) / TILE_SIZE);
    ...
    if (decision_type != AI_DECISION_REVIVE) {
        ...
        if (player_index == our_index) {
            if (player.game_mode == GAME_MODE_MP_ADVENTURE) {
                ...
                }while (!(decision_target.x >= 0 && decision_target.x <= (level.level_x / TILE_SIZE) - 1 &&
                          decision_target.y >= 0 && decision_target.y <= (level.level_y / TILE_SIZE) - 1));
        ...
        if (decision_type == AI_DECISION_FOLLOW) {
            ...
            if (player_index >= 0) {
                PLAYER_MOVING = mp_players[player_index].IN_AIR || mp_players[player_index].underwater ||
                                mp_players[player_index].run_speed != 0.0;
            }
...
bool Player_Mp::ai_tile_is_air (Tile* tile) {
    if (tile->solidity == TILE_SOLIDITY_PASSABLE &&
        (tile->special == TILE_SPECIAL_NONE || tile->special == TILE_SPECIAL_MIRROR ||
         tile->special == TILE_SPECIAL_ITEM_HOLDER)) {
        return true;
    } else {
        return false;
    }
    ...
    if (ai_controlled) {
        ...
        if (!DYING && !bubble_mode) {
            for (int i = path.size() - 1; i >= 0; i--) {
                ...
                if (check_for_collide && collision_check(x, y, w, h, path[i].coords.x * TILE_SIZE + TILE_SIZE / 4,
                                                         path[i].coords.y * TILE_SIZE + TILE_SIZE / 4, TILE_SIZE / 2,
                                                         TILE_SIZE / 2)) {
                    counter_path_giveup = AI_PATH_GIVEUP;
            ...
            if (player.cam_focused_dying() ||
                (fabs(x - player.cam_focused_x()) < AI_BUBBLE_POP_RANGE &&
                 fabs(y - player.cam_focused_y()) < AI_BUBBLE_POP_RANGE)) {
...
bool Player_Mp::ai_tile_valid (int int_x, int int_y, int target_x, int target_y) {
    ...
    if (tile->solidity != TILE_SOLIDITY_SOLID && tile->special != TILE_SPECIAL_HAZARD) {
        if (tile->special != TILE_SPECIAL_WATER ||
            (tile->special == TILE_SPECIAL_WATER &&
             (!player.deadly_water || (player.deadly_water && suit_deadly_water)) && (swimming_gear || underwater))) {
            return true;
        }
    }
...
int Player_Mp::ai_get_gravity_cost (int int_x, int int_y) {
    ...
    for (int check_y = int_y; check_y <= int_y + 5; check_y++) {
        if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && check_y >= 0 &&
            check_y <= (level.level_y / TILE_SIZE) - 1) {
            Tile* tile = &level.tile_array[int_x][check_y];

            if (tile->solidity == TILE_SOLIDITY_SOLID || tile->solidity == TILE_SOLIDITY_CLOUD ||
                tile->special == TILE_SPECIAL_CLIMBABLE_TOP || tile->special == TILE_SPECIAL_WATER) {
                break;
            }
...
void Player_Mp::ai_add_open_tiles (Path_Calc* parent, int target_x, int target_y, vector<Path_Calc>* open_list,
                                   vector<Path_Calc>* closed_list, vector<Path_Calc>* new_open_tiles) {
        ...
        for (int int_y = parent->coords.y - 1; int_y <= parent->coords.y + 1; int_y++) {
            if (int_x != parent->coords.x || int_y != parent->coords.y) {
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if (on_open_list == -1 && ai_tile_on_list(closed_list, int_x, int_y) == -1 && ai_tile_valid(int_x,
                                                                                                                int_y,
                                                                                                                target_x,
                                                                                                                target_y))
                    {
                        ...
                        new_open_tiles->push_back(Path_Calc(int_x, int_y, parent->coords.x, parent->coords.y, cost_g,
                                                            cost_g + cost_h));
...
void Player_Mp::ai_path_to_target (int target_x, int target_y) {
    ...
    if (target_x >= 0 && target_x <= (level.level_x / TILE_SIZE) - 1 && target_y >= 0 &&
        target_y <= (level.level_y / TILE_SIZE) - 1 && current_x >= 0 && current_x <= (level.level_x / TILE_SIZE) - 1 &&
        current_y >= 0 && current_y <= (level.level_y / TILE_SIZE) - 1 &&
        ...
        for (int tile_checks = 0; tile_checks < path_range; tile_checks++) {
            ...
            for (int i = 0; i < open_list.size(); i++) {
                if (lowest_f_cost == 0 || (open_list[i].coords.x == target_x && open_list[i].coords.y == target_y) ||
                    open_list[i].cost_f <= lowest_f_cost->cost_f) {
            ...
            if (lowest_f_cost != 0) {
                ...
                if (lowest_f_cost->coords.x == target_x && lowest_f_cost->coords.y == target_y) {
                    ...
                    do{
                        ...
                        path_tile = ai_tile_on_list(&closed_list, closed_list[path_tile].parent.x,
                                                    closed_list[path_tile].parent.y);
                    }while (path_tile != -1);
...
void Player_Mp::ai_move_to_path () {
    if (path.size() > 0) {
        ...
        if (SWIMMING) {
            ...
            if (touching_sloped_ground && path.size() > 1 &&
                ...
                ai_keystates[COMMAND_JUMP] = true;
                ai_key_events.push_back(COMMAND_JUMP);
            } else if (on_climbable_top_tile && path.size() > 1 &&
                       (level.tile_array[path[path.size() - 2].coords.x][path[path.size() - 2].coords.y].special ==
                ...
                ai_keystates[COMMAND_DOWN] = true;
                       ...
                       (level.tile_array[next_path->coords.x][next_path->coords.y].special == TILE_SPECIAL_CLIMBABLE ||
                        level.tile_array[next_path->coords.x][next_path->coords.y].special ==
                        TILE_SPECIAL_CLIMBABLE_TOP)) {
                lets_climb = true;
                ai_keystates[COMMAND_UP] = true;
            } else if (y >= next_coord_y &&
                       ((run_speed < 0 && x >= next_coord_x) || (run_speed > 0 && x < next_coord_x)) &&
                       (!touching_sloped_ground || (touching_sloped_ground && fabs(x - next_coord_x) < TILE_SIZE))) {
                ai_keystates[COMMAND_JUMP] = true;
                ai_key_events.push_back(COMMAND_JUMP);
            }
...
void Player_Mp::ai_jump () {
    if (path.size() > 0) {
        if (jump_state && air_velocity < 0.0) {
            if (IN_AIR &&
                ((path.size() == 1 && y >= path[path.size() - 1].coords.y * TILE_SIZE + TILE_SIZE / 2) ||
        ...
        if (!jump_state && j_wing && extra_jumps > 0 &&
            ((path.size() == 1 && y >= path[path.size() - 1].coords.y * TILE_SIZE + TILE_SIZE / 2) ||
             (path.size() > 1 && y >= path[path.size() - 2].coords.y * TILE_SIZE + TILE_SIZE / 2))) {
            ai_keystates[COMMAND_JUMP] = true;
            ai_key_events.push_back(COMMAND_JUMP);
