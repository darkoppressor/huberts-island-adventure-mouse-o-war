                ...
                for (int i = ACHIEVEMENT_BEGIN; i < ACHIEVEMENT_END; i++) {
                    ...
                    if (!player.achievements[i]) {
                        ...
                        if (i == ACHIEVEMENT_COLLECT_ALL_LEAVES_IN_LEVEL) {
                            if (!player.on_worldmap() && level.leaves != 0 &&
                                player.current_level_leaves >= level.leaves) {
                                earn_achievement(i);
                            }
                        } else if (i == ACHIEVEMENT_COLLECT_ALL_CHEESE_IN_LEVEL) {
                            if (!player.on_worldmap() && level.cheese != 0 &&
                                player.current_level_cheese >= level.cheese) {
                                earn_achievement(i);
                            }
                        ...
                        // *********//
                            ...
                            if (player.stat_deaths_enemies + player.stat_deaths_traps + player.stat_deaths_drowning >=
                                1) {
                                earn_achievement(i);
                            }
