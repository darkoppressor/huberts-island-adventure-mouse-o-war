...
void BitmapFont::build_font (string font_location, double get_w, double get_h, short get_spacing_x,
                             short get_spacing_y) {
...
void BitmapFont::show (double x, double y, string text, short font_color, double opacity, double scale) {
    ...
    for (short show = 0; text[show] != '\0'; show++) {
        ...
        if (text[show] != '\xA') {
            if (X + scaled_spacing_x >= 0 && X <= main_window.SCREEN_WIDTH && Y + scaled_spacing_y >= 0 &&
                Y <= main_window.SCREEN_HEIGHT) {
                // Show the character.
                render_sprite((int) X, (int) Y, bitmap_font_texture, &chars[ascii], opacity, scale, scale, 0.0,
                              font_color);
            }
