...
bool Level::return_title_pause () {
    if (level.title_moving == TITLE_BG_MOVING_IN || level.title_moving == TITLE_MOVING_IN ||
        level.title_moving == TITLE_WAITING) {
        return true;
    } else {
        return false;
    }
...
void Level::process_title () {
    ...
    if (title_moving == TITLE_BG_MOVING_IN) {
        ...
        if (title_bottom_x >=
            (main_window.SCREEN_WIDTH - title_bottom_string.length() * font_large.spacing_x * 2.0) / 2.0) {
        ...
        if (title_bg_y_top + main_window.SCREEN_HEIGHT < 0 && title_bg_x_bottom > main_window.SCREEN_WIDTH &&
            title_bg_x_left + main_window.SCREEN_WIDTH < 0) {
        ...
        if (title_top_x > main_window.SCREEN_WIDTH &&
            title_bottom_x + title_bottom_string.length() * font_large.spacing_x * 2.0 < 0) {
            title_moving = TITLE_NOT_MOVING;
        }
    }
}
...
void Level::render_title () {
    if (title_moving != TITLE_NOT_MOVING) {
        ...
        font_large.show(title_top_x + 4,
                        (main_window.SCREEN_HEIGHT - font_large.spacing_y * 2.0) / 2.0 - font_large.spacing_y * 2.0 + 4,
                        title_top_string, COLOR_BLACK, 1.0, 2.0);
        font_large.show(title_top_x,
                        (main_window.SCREEN_HEIGHT - font_large.spacing_y * 2.0) / 2.0 - font_large.spacing_y * 2.0,
                        title_top_string, return_gui_color(holiday, 3), 1.0, 2.0);

        font_large.show(title_bottom_x + 4,
                        (main_window.SCREEN_HEIGHT - font_large.spacing_y * 2.0) / 2.0 + font_large.spacing_y * 2.0 + 4,
                        title_bottom_string, COLOR_BLACK, 1.0, 2.0);
        font_large.show(title_bottom_x,
                        (main_window.SCREEN_HEIGHT - font_large.spacing_y * 2.0) / 2.0 + font_large.spacing_y * 2.0,
                        title_bottom_string, return_gui_color(holiday, 3), 1.0, 2.0);
    }
}
...
void Level::load_level_adventure () {
    ...
    if (player.previous_level != -1 &&
        (player.previous_level != player.current_level || player.previous_sub_level != player.current_sub_level)) {
    ...
    ss.clear();
    ss.str("");
    ss << player.current_sub_level;
    current_sub_level = ss.str();

    // Clear the vectors. This ensures that any previous data held in the vectors is deleted, and that any memory that
    // was allocated by the objects in the vectors is freed.
    background_layers.clear();
    ...
    if (load.is_opened()) {
        ...
        for (int int_y = 0; int_y < level_y / TILE_SIZE; int_y++) {
            for (int int_x = 0; int_x < level_x / TILE_SIZE; int_x++) {
                ...
                // Then generate a new tile in the tile vector with the proper coordinates and the type loaded from the
                // level file.
        ...
        if (load.is_opened()) {
            ...
            for (int int_y = 0; int_y < (level_y / TILE_SIZE); int_y++) {
                for (int int_x = 0; int_x < (level_x / TILE_SIZE); int_x++) {
                    ...
                    // Then generate a new tile in the tile vector with the proper coordinates and the type loaded from
                    // the level file.
            ...
            while (!data_stream.eof()) {
                ...
                if (!player.game_mode_is_multiplayer() || player.game_mode == GAME_MODE_MP_ADVENTURE) {
                    ...
                    if (player.npc_is_main_boss(type) &&
                        (player.bosses[player.current_level] == BOSS_STATE_NOT_SPAWNED ||
    ...
    if (!player.on_worldmap()) {
        ...
        if (load.is_opened()) {
            ...
            while (!data_stream.eof()) {
                ...
                if (move_speed != 0) {
                    vector_moving_platforms.push_back(Moving_Platform(move_speed, start_point_x, start_point_y,
                                                                      end_point_x, end_point_y, waypoints, active,
                                                                      round_trip, movement_type, identifier));
                }
            }
    ...
    if (!player.on_worldmap()) {
        ...
        if (load.is_opened()) {
            ...
            while (!data_stream.eof()) {
                ...
                if (w != 0) {
                    vector_triggers.push_back(Trigger(x, y, w, h, targets, trigger_method, user_type, repeating,
                                                      repeat_time, render_trigger));
                }
            }
    ...
    for (int i = 0; i < vector_triggers.size() && !weather_machine_check; i++) {
        for (int n = 0; n < vector_traps.size() && !weather_machine_check; n++) {
            if (vector_traps[n].type == TRAP_WEATHER_MACHINE && fabs(vector_traps[n].x - vector_triggers[i].x) < 128 &&
                fabs(vector_traps[n].y - vector_triggers[i].y) < 128) {
    ...
    if (player.previous_level != player.current_level || player.previous_sub_level != player.current_sub_level) {
        ...
        if (player.current_sub_level == 0) {
            ...
            music.load_track("data/music/" + current_level + "/" + current_sub_level + "/normal.ogg",
                             MUSIC_LEVEL_NORMAL);
            render_loading_screen((double) ++things_loaded / (double) things_to_load, player.current_level);
        ...
        if (!player.on_worldmap()) {
            ...
            title_bottom_x = 0.0 - title_bottom_string.length() * font_large.spacing_x * 2.0 - font_large.spacing_x *
                             2.0;
    ...
    if (player.get_upgrade_state("monkeys")) {
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (!vector_npcs[i].BOSS && !vector_npcs[i].counts_as_trap && !vector_npcs[i].friendly &&
                vector_npcs[i].type != NPC_MONKEY) {
    ...
    if (difficulty == DIFFICULTY_EASY) {
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (random_range_pass(&temp_rng.mrand_main, 0,
                                  99) < 25 && !vector_npcs[i].BOSS && vector_npcs[i].type != NPC_SALLY &&
        ...
        for (int i = 0; i < vector_npcs.size(); i++) {
            ...
            if (!vector_npcs[i].BOSS && vector_npcs[i].exists && !vector_npcs[i].counts_as_trap &&
                !vector_npcs[i].friendly && vector_npcs[i].can_be_duplicated && !on_list) {
                indexes.push_back(i);
            }
        }
                ...
                while (emergency_loop_break < 5000) {
                    ...
                    if (x >= 0 && x <= (level_x / TILE_SIZE) - 1 && y >= 0 && y <= (level_y / TILE_SIZE) - 1) {
                        ...
                        for (int items = 0; items < vector_items.size(); items++) {
                            if ((vector_items[items].type == ITEM_SPAWNPOINT ||
                                 vector_items[items].type == ITEM_CHECKPOINT ||
                                 vector_items[items].type == ITEM_ENDPOINT) &&
                                abs(vector_items[items].x - x * TILE_SIZE) < spawn_protection &&
                                abs(vector_items[items].y - y * TILE_SIZE) < spawn_protection) {
                                near_spawn = true;
                            ...
                            if (npc->FLYING) {
                                ...
                                for (int int_x = x; int_x < x + npc_width && !invalid_tile; int_x++) {
                                    for (int int_y = y; int_y < y + npc_height && !invalid_tile; int_y++) {
                                        if (int_x >= 0 && int_x <= (level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                                            int_y <= (level_y / TILE_SIZE) - 1) {
                                            Tile* tile_check = &tile_array[int_x][int_y];

                                            if (tile_check->solidity != TILE_SOLIDITY_PASSABLE ||
                                                tile_check->special == TILE_SPECIAL_HAZARD ||
                                ...
                                if (!invalid_tile) {
                                    if (y + npc_height <= (level_y / TILE_SIZE) - 1 &&
                                        tile_array[x][y + npc_height].solidity != TILE_SOLIDITY_PASSABLE &&
                                        tile_array[x][y + npc_height].slope == 180) {
                                        location_valid = true;
                                    }
                                }
                            }
...
Light_Data Level::blend_light (double src_opacity, color_data src_color, double dest_opacity, color_data dest_color) {
    ...
    result_light.color.red = ((src_color.red * src_opacity) + (dest_color.red * (dest_opacity * (1.0 - src_opacity)))) /
                             result_light.light_intensity;
    result_light.color.green = ((src_color.green * src_opacity) +
                                (dest_color.green * (dest_opacity * (1.0 - src_opacity)))) /
                               result_light.light_intensity;
    result_light.color.blue = ((src_color.blue * src_opacity) +
                               (dest_color.blue * (dest_opacity * (1.0 - src_opacity)))) / result_light.light_intensity;
