...
Trigger::Trigger (double get_x, double get_y, double get_w, double get_h, vector<target> get_targets,
                  bool get_trigger_method, short get_user_type, bool get_repeating, short get_repeat_time,
                  short get_render_trigger) {
    x = get_x;
    y = get_y;
            ...
            if (user_type == TRIGGER_USER_ALL) {
                ...
                for (int i = 0; i < vector_npcs.size(); i++) {
                    ...
                    if (vector_npcs[i].exists) {
                        if (collision_check(x, y, w, h, vector_npcs[i].x, vector_npcs[i].y, vector_npcs[i].w,
                                            vector_npcs[i].h)) {
                            use();
                        }
                    }
    ...
    if (render_trigger != 0) {
        ...
        if (x >= player.camera_x - w && x <= player.camera_x + player.camera_w && y >= player.camera_y - h &&
            ...
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_triggers,
                          &sprites_triggers[render_trigger - 1].change_states[frame]);
        }
    }
}
