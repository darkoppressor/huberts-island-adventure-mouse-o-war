...
Button::Button (short get_x, short get_y, string get_tooltip_text, string get_text,
                void(*get_event_function) (Window*, int), sound_data* get_mouse_over_sound,
                sound_data* get_event_fire_sound, bool get_invisible, int get_special_data, bool get_enabled) {
    x = get_x;
    y = get_y;
...
bool Button::is_moused_over (int mouse_x, int mouse_y, short x_offset, short y_offset, int index) {
    if (index == player.current_button || (player.mouse_allowed() && collision_check(mouse_x, mouse_y, 2, 2,
                                                                                     x_offset + x, y_offset + y, w,
                                                                                     h))) {
        return true;
    }
