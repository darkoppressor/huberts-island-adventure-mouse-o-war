...
void Player_Mp::handle_command_event (short command) {
    switch (command) {
        case COMMAND_JUMP:
            ...
            // As long as the game is not paused, and is in progress.
                ...
                if (!DYING) {
                    ...
                    if (command_state(COMMAND_JUMP) && command_state(COMMAND_DOWN) && !IN_AIR && !SHOOTING &&
                        !CLIMBING) {
                        if (on_cloud) {
                            fall_through_cloud();
                    ...
                    if (!fell_through_cloud &&
                        (counter_jump_mode > 0 || (extra_jumps > 0 && ((SWIMMING && SWIM_CAN_JUMP) || !SWIMMING)) ||
                ...
                if (collision_check(x, y, w, h, player.camera_x, player.camera_y, player.camera_w,
                                    player.camera_h) && in_open_air()) {
                    bubble_mode = false;
        ...
        case COMMAND_SHOOT:
            ...
            // As long as the game is not paused and is in progress.
                ...
                if (!SWIMMING && !SHOOTING && !DYING) {
                    ...
                    // Pretty straightforward. If shoot_state is non-zero, then the player is holding a directional key,
                    // so shoot in the corresponding direction and subtract one ammo.
        ...
        case COMMAND_UP: case COMMAND_LEFT:
            ...
            // If a window is open.
            if (window_manager.which_window_open() != -1) {} else {
