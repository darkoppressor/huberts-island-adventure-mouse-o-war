        ...
        for (int i = 0; i < vector_triggers.size(); i++) {
            if (fabs(vector_triggers[i].x - x) < PROCESS_RANGE && fabs(vector_triggers[i].y - y) < PROCESS_RANGE) {
                if (type != NPC_REPLICATOR) {
                    ...
                    if (vector_triggers[i].active && (vector_triggers[i].user_type == TRIGGER_USER_ALL) &&
                        !vector_triggers[i].trigger_method) {
                ...
                // Special Replicator-only way to use triggers.
                    ...
                    if (vector_triggers[i].active && !vector_triggers[i].trigger_method) {
                        if (collision_check(x - TILE_SIZE * 4, y - TILE_SIZE * 4, w + TILE_SIZE * 8, h + TILE_SIZE * 8,
                                            vector_triggers[i].x, vector_triggers[i].y, vector_triggers[i].w,
                                            vector_triggers[i].h)) {
                            vector_triggers[i].use();
                            play_positional_sound(sound_system.trigger_click, vector_triggers[i].x,
                                                  vector_triggers[i].y);
...
void Npc::move () {
    ...
    if (!never_move) {
        ...
        if (exists && !FLYING) {
            ...
            if (SWIMMING && move_state != UP && move_state != LEFT_UP && move_state != RIGHT_UP && move_state != DOWN &&
                move_state != LEFT_DOWN && move_state != RIGHT_DOWN) {
                // Float upwards.
                ...
                if (!CLIMBING) {
                    ...
                    if (level.tile_array[int_x][int_y].special == TILE_SPECIAL_HAZARD) {
                        if (collision_check(return_x(), return_y(), return_w(), return_h(),
                                            level.tile_array[int_x][int_y].return_x(),
                                            level.tile_array[int_x][int_y].return_y() + 6, TILE_SIZE, TILE_SIZE - 6)) {
                            ///I need to decide what happens to NPCs that touch hazards.
                            handle_death();
                        }
                    }
                    // If the tile is a water tile and water tiles are deadly.
                    else if (player.deadly_water && SWIMMING &&
            ...
            if (this != &vector_npcs[i] && exists && vector_npcs[i].exists) {
                if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                    if (!CLIMBING && !vector_npcs[i].CLIMBING) {
                        if (ethereal || vector_npcs[i].ethereal || ethereal_to_npcs ||
                            vector_npcs[i].ethereal_to_npcs) {
                            solidity = TILE_SOLIDITY_PASSABLE;
                        ...
                        if (!vector_npcs[i].SWIMMING && act_as_platform && !ethereal && !vector_npcs[i].ethereal) {
                            if (push_actor_up(&vector_npcs[i], vector_npcs[i].x, vector_npcs[i].w, being_pushed_up,
                                              type)) {
                    ...
                    /**if(vector_npcs[i].exists && vector_npcs[i].deadly_to_touch &&
                       collision_check(x,y,w,h,vector_npcs[i].x,vector_npcs[i].y,vector_npcs[i].w,vector_npcs[i].h)){
                        //The npc dies!
                        handle_death();
                       }*/
                }
            ...
            for (int i = 0; i < mp_players.size(); i++) {
                if (fabs(mp_players[i].x - x) < PROCESS_RANGE && fabs(mp_players[i].y - y) < PROCESS_RANGE) {
                    if (!CLIMBING && !mp_players[i].DYING && !mp_players[i].bubble_mode && !mp_players[i].CLIMBING) {
                        ...
                        // Check for a collision with the player.
                        handle_collision_solid(mp_players[i].x, mp_players[i].y, mp_players[i].w, mp_players[i].h,
                                               solidity, NPC_END);

                        // If this npc is acting as a platform.
                        if (!mp_players[i].SWIMMING && act_as_platform && !ethereal) {
                            if (push_actor_up(&mp_players[i], mp_players[i].x, mp_players[i].w, being_pushed_up,
                                              type)) {
...
void Npc::handle_death (bool override_invulnerability) {
    if (BOSS) {
        // We do nothing if override_invulnerability is true, because bosses should ONLY die if truly killed by the
        // player.
            ...
            if (exists && !invulnerable) {
                ...
                if (health <= 0) {
                    ...
                    for (int i = 0; i < number_of_items; i++) {
                        vector_items.push_back(Item(x + (w - 16) / 2.0, y + (h - 16) / 2.0, true, ITEM_CANDY, 0, false,
                                                    1, 100, 1, 175, true, 15, score_bonus));
                    }

                    new_shots.push_back(new_shot_data(x + (w - 128) / 2.0, y + (h - 128) / 2.0, 0.0, SHOT_BOSS_DEFEATED,
                                                      false));
                    ...
                    if (MAIN_BOSS) {
                        if (player.return_main_bosses_defeated() == 4) {
                            ...
                            // Play story message.
                            window_message[0].set_message(player.boss_messages[14].message_title,
                                                          player.boss_messages[14].message);
                ...
                if (drops_items) {
                    ...
                    if (random_type < 1) {
                        ...
                        for (int i = 0; i < random; i++) {
                            vector_items.push_back(Item(x + w / 2.0, y - 16, true, ITEM_AMMO, 0, false, 20, 40, 20,
                                                        80));
                        }
                    } else {
                        int random = 0;
                        ...
                        for (int i = 0; i < random; i++) {
                            vector_items.push_back(Item(x + w / 2.0, y - 16, true, ITEM_CANDY, 0, false, 20, 40, 20,
                                                        80));
                        }
                    }
                }
    ...
    if (exists) {
        ...
        if (frame_counter >= frame_speed_walk) {
            ...
            if (has_footsteps) {
                ...
                if (!SWIMMING && !IN_AIR && !SHOOTING && !CLIMBING && !DYING && run_speed != 0.0 &&
                    frame % footstep_sound_frequency == 0) {
                    ///play_footstep_sound();
                }
            }
        ...
        if (frame_counter_special_attack >= frame_speed_special_attack && special_attack_in_progress) {
            ...
            if (special_attack_type == SPECIAL_ATTACK_MONKEY_THROW_FRUIT &&
                frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a banana shot.

                // Determine the angle to the player.
                double angle = ai_get_target_direction(special_attack_target_x, special_attack_target_y, PLAYER_W,
                                                       PLAYER_H);
                double spawn_x;
                double spawn_y;
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_LASER_SPINNER_FIRE_LASER &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a laser shot.
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_BALL_KNIGHT_THROW_BALL &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a ball shot.
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_ARCHER_FIRE_ARROW &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_SNOWMAN_THROW_SNOWBALL &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_DRAGON_WHELP_SHOOT_FIREBALL &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_FIREBALL_SHOOTER_SHOOT_LEFT &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a fireball shot.
                ...
                vector_shots.push_back(Shot(spawn_x, spawn_y, angle, SHOT_FIREBALL, counts_as_trap));
            } else if (special_attack_type == SPECIAL_ATTACK_FIREBALL_SHOOTER_SHOOT_RIGHT &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a fireball shot.
                ...
                vector_shots.push_back(Shot(spawn_x, spawn_y, angle, SHOT_FIREBALL, counts_as_trap));
            } else if (special_attack_type == SPECIAL_ATTACK_FIREBALL_SHOOTER_SHOOT_UP &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a fireball shot.
                ...
                vector_shots.push_back(Shot(spawn_x, spawn_y, angle, SHOT_FIREBALL, counts_as_trap));
            } else if (special_attack_type == SPECIAL_ATTACK_FIREBALL_SHOOTER_SHOOT_DOWN &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a fireball shot.
                ...
                vector_shots.push_back(Shot(spawn_x, spawn_y, angle, SHOT_FIREBALL, counts_as_trap));
            } else if (special_attack_type == SPECIAL_ATTACK_SCORPION_SHOOT_POISON &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_MECHSUIT_MOUSE_FIRE_MISSILE &&
                       frame_special_attack == NPC_SPECIAL_ATTACK_SPRITES - 1) {
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_BOSS_HAUNTED_HOUSE &&
                       frame_special_attack >= NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a boss shot.
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_BOSS_VOLCANO &&
                       frame_special_attack >= NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a boss shot.
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_BOSS_PYRAMID &&
                       frame_special_attack >= NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a boss shot.
            ...
            } else if (special_attack_type == SPECIAL_ATTACK_BOSS_LIGHTHOUSE &&
                       frame_special_attack >= NPC_SPECIAL_ATTACK_SPRITES - 1) {
                // Create a boss shot.
            ...
            if (has_footsteps) {
                // If the actor is walking, play an occasional footstep sound.
                if (!SWIMMING && !IN_AIR && !SHOOTING && !CLIMBING && !DYING && run_speed != 0.0 &&
                    frame_special_attack % footstep_special_attack_sound_frequency == 0) {
                    ///play_footstep_sound();
                }
            }
...
void Npc::render (bool mirrored) {
    ...
    if (!mirrored || (mirrored && !undead)) {
        ...
        if (exists) {
            ...
            if (x >= player.camera_x - w && x <= player.camera_x + player.camera_w && y >= player.camera_y - h &&
                y <= player.camera_y + player.camera_h) {
                ...
                if (!SWIMMING && !IN_AIR && !special_attack_in_progress && !FLYING && NPC_WALK_SPRITES > 0) {
                    if ((move_state == 0 || move_state == UP || move_state == DOWN) && facing == LEFT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_left[0], 1.0, scale_x, scale_y, 0.0, color);
                    } else if ((move_state == 0 || move_state == UP || move_state == DOWN) && facing == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_right[0], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == LEFT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_left[frame], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_right[frame], 1.0, scale_x, scale_y, 0.0, color);
                    }
                }
                // If the npc is performing its special attack.
                    ...
                    if (facing == LEFT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_special_attack_left[frame_special_attack], 1.0, scale_x, scale_y,
                                      0.0, color);
                    } else if (facing == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_special_attack_right[frame_special_attack], 1.0, scale_x, scale_y,
                                      0.0, color);
                    }
                }
                // If the npc is flying.
                    ...
                    if (move_state == 0 && facing == LEFT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_fly_left[frame_fly], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == 0 && facing == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_fly_right[frame_fly], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == LEFT || move_state == LEFT_DOWN || move_state == LEFT_UP ||
                               ((move_state == UP || move_state == DOWN) && facing == LEFT)) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_fly_left[frame_fly], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == RIGHT || move_state == RIGHT_DOWN || move_state == RIGHT_UP ||
                               ((move_state == UP || move_state == DOWN) && facing == RIGHT)) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_fly_right[frame_fly], 1.0, scale_x, scale_y, 0.0, color);
                    }
                ...
                // If the npc is swimming.
                    ...
                    if (move_state == 0 && facing == LEFT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_swim_left[frame_swim], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == 0 && facing == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_swim_right[frame_swim], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (move_state == LEFT || move_state == LEFT_DOWN || move_state == LEFT_UP ||
                               ((move_state == UP || move_state == DOWN) && facing == LEFT)) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                ...
                // If the npc is in the air.
                    ...
                    if (facing == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_jump_right[frame_jump], 1.0, scale_x, scale_y, 0.0, color);
                    } else if (facing == LEFT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_jump_left[frame_jump], 1.0, scale_x, scale_y, 0.0, color);
                    }
                }
            ...
            if (x >= player.camera_x - w && x <= player.camera_x + player.camera_w && y >= player.camera_y - h &&
                ...
                if (NPC_DEAD_SPRITES > 0) {
                    if (facing == RIGHT) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_npc,
                                      &vector_sprites_dead[frame_death], opacity, scale_x, scale_y, 0.0, color);
                    } else if (facing == LEFT) {
                        render_sprite((int) ((int) render_x -
                                             (NPC_SPRITE_COUNTS[type].dead_width - NPC_SPRITE_COUNTS[type].width) -
                                             (int) player.camera_x), (int) ((int) render_y - (int) player.camera_y),
                                      *sprite_sheet_image_npc, &vector_sprites_dead[frame_death], opacity, scale_x,
                                      scale_y, 0.0, color, true);
                    }
                }
            }
        }
    ...
    if (wears_tophat && (exists || (!exists && tophat_when_dead)) && !counts_as_trap &&
        player.get_upgrade_state("tophats")) {
        SDL_Rect temp;
        ...
        render_sprite((int) ((int) hat_x - (int) player.camera_x), (int) ((int) hat_y - (int) player.camera_y),
                      image.tophat, &temp, opacity, hat_scale_x, hat_scale_y, 0.0, COLOR_WHITE, flip_x);
    }
