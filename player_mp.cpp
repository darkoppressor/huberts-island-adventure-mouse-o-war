...
void Player_Mp::move () {
    ...
    if (!bubble_mode) {
        if (player.game_mode_is_multiplayer()) {
            if (!DYING && which_mp_player != player.cam_focused_index() && !collision_check(x, y, w, h, player.camera_x,
                                                                                            player.camera_y,
                                                                                            player.camera_w,
                                                                                            player.camera_h)) {
                put_in_bubble();
    ...
    // If in bubble mode.
        ...
        if (!player.cam_focused_dying() && collision_check(x + w / 4.0, y + h / 8.0, w / 2.0, h / 4.0,
                                                           player.cam_focused_x() + player.cam_focused_w() / 4.0,
                                                           player.cam_focused_y() + player.cam_focused_h() / 8.0,
                                                           player.cam_focused_w() / 2.0,
                                                           player.cam_focused_h() / 4.0)) {
            bubble_mode = false;
...
void Player_Mp::handle_tile_hazard (int int_x, int int_y) {
    ...
    else if (player.deadly_water && !suit_deadly_water && SWIMMING &&
...
void Player_Mp::handle_events (bool being_pushed_up) {
    ...
    if (!DYING) {
                ...
                if (vector_items[i].exists) {
                    if (collision_check(x, y, w, h, vector_items[i].x, vector_items[i].y, vector_items[i].w,
                        ...
                        if (vector_items[i].type == ITEM_LEAF) {
                            player.gain_score(SCORES_ITEMS[ITEM_LEAF], vector_items[i].x + vector_items[i].w / 2.0,
                                              vector_items[i].y);
                            ...
                            player.gain_score(SCORES_ITEMS[ITEM_CHEESE], vector_items[i].x + vector_items[i].w / 2.0,
                                              vector_items[i].y);
                            ...
                            int ammo_per_player = floor((double) player.return_ammo_box_amount() /
                                                        (double) player_count);
                            ...
                            player.gain_score(SCORES_ITEMS[ITEM_J_BALLOON], vector_items[i].x + vector_items[i].w / 2.0,
                                              vector_items[i].y);
                            ...
                            int player_count = 1 + mp_players.size();
                            int ammo_per_player = floor((double) player.return_ammo_barrel_amount() /
                                                        (double) player_count);
                            ...
                            player.gain_score(player.return_candy_score(vector_items[i].score_bonus),
                            ...
                            if (next_available_slot != -1) {
                                ...
                                player.inventory[player.inventory.size() -
                                                 1].name = player.name_inventory_item(vector_items[i].type);
                                ...
                                if (vector_items[i].type == ITEM_SWIMMING_GEAR) {
                                    ...
                                    player.gain_score(SCORES_ITEMS[ITEM_SWIMMING_GEAR],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_RED) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_RED],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_BLUE) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_BLUE],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_GREEN) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_GREEN],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_YELLOW) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_YELLOW],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_ORANGE) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_ORANGE],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_PURPLE) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_PURPLE],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_TOWEL) {
                                    player.gain_score(SCORES_ITEMS[ITEM_TOWEL],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_GRAY) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_GRAY],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_BROWN) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_BROWN],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_BLACK) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_BLACK],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_PINK) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_PINK],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_CYAN) {
                                    player.gain_score(SCORES_ITEMS[ITEM_KEY_CYAN],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SINK) {
                                    player.gain_score(SCORES_ITEMS[ITEM_SINK],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SUIT_DEADLY_WATER) {
                                    new_special_item = true;

                                    player.gain_score(SCORES_ITEMS[ITEM_SUIT_DEADLY_WATER],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SUIT_SHARP) {
                                    new_special_item = true;

                                    player.gain_score(SCORES_ITEMS[ITEM_SUIT_SHARP],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SUIT_BANANA) {
                                    new_special_item = true;

                                    player.gain_score(SCORES_ITEMS[ITEM_SUIT_BANANA],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SHOT_HOMING) {
                                    new_special_item = true;

                                    player.gain_score(SCORES_ITEMS[ITEM_SHOT_HOMING],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_TRANSLATOR) {
                                    new_special_item = true;

                                    player.gain_score(SCORES_ITEMS[ITEM_TRANSLATOR],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_J_WING) {
                                    new_special_item = true;

                                    player.gain_score(SCORES_ITEMS[ITEM_J_WING],
                                                      vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                }
            ...
            if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                if (vector_npcs[i].exists) {
                    if (!CLIMBING && !vector_npcs[i].CLIMBING) {
                        ...
                        handle_collision_solid(vector_npcs[i].x, vector_npcs[i].y, vector_npcs[i].w, vector_npcs[i].h,
                                               solidity, vector_npcs[i].type);
        ...
        for (int i = 0; i < mp_players.size(); i++) {
            if (this != &mp_players[i] && !CLIMBING && !CROUCHING && !mp_players[i].DYING &&
                !mp_players[i].bubble_mode && !mp_players[i].CLIMBING && !mp_players[i].CROUCHING) {
                short solidity;
...
void Player_Mp::handle_death (double death_x, double death_y, double death_w, double death_h,
...
void Player_Mp::render (bool mirrored) {
    ...
    // If the player is dying.
    if (DYING) {
        render_sprite((int) ((int) render_x - (int) player.camera_x), (int) ((int) render_y - (int) player.camera_y),
    ...
    // If the player is on the ground.
        ...
        if (run_speed == 0.0 && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_idle_left[frame_idle]);
        } else if (run_speed == 0.0 && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_idle_right[frame_idle]);
        } else if (run_speed != 0.0 && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_left[frame]);
        } else if (run_speed != 0.0 && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_right[frame]);
        }
    }
    // If the player is crouching.
        ...
        if (facing == LEFT || facing == LEFT_UP || facing == LEFT_DOWN) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_crouch_left[0]);
        } else if (facing == RIGHT || facing == RIGHT_UP || facing == RIGHT_DOWN) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_crouch_right[0]);
        }
    }
    // If the player is swimming.
    else if (SWIMMING) {
        if (move_state == 0 && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_swim_left[frame_swim]);
        } else if (move_state == 0 && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_swim_right[frame_swim]);
        } else if (move_state == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_swim_up_right[frame_swim]);
        } else if (move_state == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_swim_up_left[frame_swim]);
    ...
    // If the player is in the air and not shooting.
        ...
        if (facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_jump_right[frame_jump]);
        } else if (facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_jump_left[frame_jump]);
        }
    }
    // If the player is shooting and not in the air or climbing.
        ...
        if (shoot_render_direction == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_up_left[frame_shoot]);
        } else if (shoot_render_direction == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_up_right[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_down_left_air[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_down_right_air[frame_shoot]);
        } else if (shoot_render_direction == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_right[frame_shoot]);
        } else if (shoot_render_direction == LEFT) {
            render_sprite((int) ((int) render_x - 4 - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_left[frame_shoot]);
        }
    }
    // If the player is shooting and in the air.
        ...
        if (shoot_render_direction == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_up_left_air[frame_shoot]);
        } else if (shoot_render_direction == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_up_right_air[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_down_left_air[frame_shoot]);
            ...
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_right_air[frame_shoot]);
        } else if (shoot_render_direction == LEFT) {
            render_sprite((int) ((int) render_x - 4 - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_left_air[frame_shoot]);
        }
    }
    // If the player is shooting and climbing.
        ...
        if (shoot_render_direction == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_climb_up_right[frame_shoot]);
        } else if (shoot_render_direction == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_climb_up_left[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_climb_down_right[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_climb_down_left[frame_shoot]);
        } else if (shoot_render_direction == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_climb_right[frame_shoot]);
        } else if (shoot_render_direction == LEFT) {
            render_sprite((int) ((int) render_x - 4 - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_shoot_climb_left[frame_shoot]);
        }
    }
    // If the player is climbing and not shooting or looking.
        ...
        if ((move_state != UP && move_state != DOWN) && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_climb_up_right[0]);
        } else if ((move_state != UP && move_state != DOWN) && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_climb_up_left[0]);
        } else if (move_state == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_climb_up_right[frame_climb]);
        } else if (move_state == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_climb_up_left[frame_climb]);
        } else if (move_state == DOWN && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_climb_down_right[frame_climb]);
        } else if (move_state == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *ptr_player_image,
                          &sprites_player_climb_down_left[frame_climb]);
        }
    }
    ...
    if (counter_jump_mode > 0 && !DYING) {
        ...
        if (render_wings) {
            render_sprite((int) ((int) x - 8 - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                          image.powerup_jump, &sprites_powerup_jump[frame_powerup], 1.0, 1.0, scale_y);
        }
    }
    ...
    if (bubble_mode && !DYING) {
        ...
        render_sprite((int) ((int) x - (balloon_width - w) / 2 - (int) player.camera_x),
                      (int) ((int) y - (balloon_height - h) / 2 - (int) player.camera_y), image.balloon, &balloon_rect,
                      1.0, balloon_scale_x, balloon_scale_y, 0.0, player.get_player_color(which_mp_player));
    }
...
void Player_Mp::render_path () {
    if (ai_controlled) {
        for (int i = 0; i < path.size(); i++) {
            ...
            if (render_x >= player.camera_x - TILE_SIZE && render_x <= player.camera_x + player.camera_w &&
                render_y >= player.camera_y - TILE_SIZE && render_y <= player.camera_y + player.camera_h) {
                render_rectangle((int) ((int) render_x - (int) player.camera_x),
                                 (int) ((int) render_y - (int) player.camera_y), TILE_SIZE, TILE_SIZE, 0.75,
                                 player.get_player_color(which_mp_player));
            }
        }

        string type_name = get_decision_type();

        font.show((int) ((int) x - ((type_name.length() * font.spacing_x) / 2) + w / 2 - (int) player.camera_x),
                  (int) ((int) y - font.spacing_y - (int) player.camera_y), type_name, COLOR_WHITE);
    }
}
