    ...
    for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
        for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
            if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                ...
                if (tile->solidity == TILE_SOLIDITY_SOLID && collision_check(x, y - h / 2, w, h / 2, tile->x,
                                                                             tile->y + TILE_SIZE / 2, TILE_SIZE,
                                                                             TILE_SIZE / 2)) {
                    solid_above = true;
...
void Actor::prepare_for_events () {
    ...
    // The rest of handle_events() will determine if the actor's feet are touching air, ground, or both, and handle
    // that.
    touching_air = false;
    ...
    if (!SWIMMING && vector_npcs[i].act_as_platform) {
        ...
        if (collision_check(x, y + h, w, 1, vector_npcs[i].x, vector_npcs[i].y, vector_npcs[i].w, 1)) {
            ...
            if ((vector_npcs[i].IN_AIR && vector_npcs[i].air_velocity < 0.0) ||
                vector_npcs[i].moving_platform_y < 0.0) {
                if (vector_npcs[i].IN_AIR && vector_npcs[i].air_velocity < 0.0) {
            ...
            } else if ((vector_npcs[i].IN_AIR && vector_npcs[i].air_velocity >= 0.0) ||
                       vector_npcs[i].moving_platform_y > 0.0) {
                if (vector_npcs[i].IN_AIR && vector_npcs[i].air_velocity >= 0.0) {
                    moving_platform_y = vector_npcs[i].air_velocity;
                } else if (vector_npcs[i].moving_platform_y > 0.0) {
...
void Actor::handle_collision_moving_platforms (short solidity, bool counts_as_trap) {
    for (int i = 0; i < vector_moving_platforms.size(); i++) {
        if (fabs(vector_moving_platforms[i].x - x) < PROCESS_RANGE &&
            fabs(vector_moving_platforms[i].y - y) < PROCESS_RANGE) {
            if (!counts_as_trap) {
                handle_collision_solid(vector_moving_platforms[i].x, vector_moving_platforms[i].y,
                                       vector_moving_platforms[i].w, vector_moving_platforms[i].h, solidity, NPC_END);

                // Regardless of the actor's IN_AIR and air_velocity status, one other check needs to be done.
                // If the top row of this npc's pixels are one pixel below the actor, he is standing on this npc.
                if (collision_check(x, y + h, w, 1, vector_moving_platforms[i].x, vector_moving_platforms[i].y,
                                    vector_moving_platforms[i].w, 1)) {
                    // Thus, we let the game know the actor is standing on some ground.
                    touching_ground = true;
                    ...
                    // X-axis.
                    if (vector_moving_platforms[i].move_state == LEFT ||
                        vector_moving_platforms[i].move_state == LEFT_DOWN ||
                        vector_moving_platforms[i].move_state == LEFT_UP) {
                        moving_platform_x = -vector_moving_platforms[i].move_speed;
                        MOVING_PLATFORM_X_THIS_FRAME = true;
                    } else if (vector_moving_platforms[i].move_state == RIGHT ||
                               vector_moving_platforms[i].move_state == RIGHT_DOWN ||
                               vector_moving_platforms[i].move_state == RIGHT_UP) {
                        moving_platform_x = vector_moving_platforms[i].move_speed;
                        MOVING_PLATFORM_X_THIS_FRAME = true;
                    }

                    // Y-axis.
                    if (vector_moving_platforms[i].move_state == UP ||
                        vector_moving_platforms[i].move_state == LEFT_UP ||
                        vector_moving_platforms[i].move_state == RIGHT_UP) {
                        moving_platform_y = -vector_moving_platforms[i].move_speed;
                    } else if (vector_moving_platforms[i].move_state == DOWN ||
                               vector_moving_platforms[i].move_state == LEFT_DOWN ||
                               vector_moving_platforms[i].move_state == RIGHT_DOWN) {
                        moving_platform_y = vector_moving_platforms[i].move_speed;
                    }
    ...
    for (int i = 0; i < vector_doors.size(); i++) {
        if (fabs(vector_doors[i].x - x) < PROCESS_RANGE && fabs(vector_doors[i].y - y) < PROCESS_RANGE) {
            ...
            if (!vector_doors[i].open) {
                ...
                if (is_player && collision_check(x - check_x, y, w + check_w, h, vector_doors[i].x, vector_doors[i].y,
                                                 vector_doors[i].w, vector_doors[i].h)) {
                    // Check if the door is a key door and should be unlocked.
                    door_unlocked = vector_doors[i].attempt_unlock();
                }

                if (!door_unlocked) {
                    handle_collision_solid(vector_doors[i].x, vector_doors[i].y, vector_doors[i].w, vector_doors[i].h,
                                           TILE_SOLIDITY_SOLID, NPC_END);
                }
            }
        }
    }
...
void Actor::handle_collision_springs (short solidity) {
    for (int i = 0; i < vector_springs.size(); i++) {
        if (fabs(vector_springs[i].x - x) < PROCESS_RANGE && fabs(vector_springs[i].y - y) < PROCESS_RANGE) {
            ...
            if (collision_check(x, y, w, h, springy.x + adjust_x, springy.y + adjust_y, springy.w + adjust_w,
                                springy.h + adjust_h)) {
                IN_AIR = true;
                frame_jump = 0;
                ...
                handle_collision_solid(vector_springs[i].x + adjust_x, vector_springs[i].y + adjust_y,
                                       SPRING_W + adjust_w, SPRING_H + adjust_h, solidity, NPC_END);
            }
        }
    }
}
...
void Actor::handle_collision_boosters () {
    for (int i = 0; i < vector_boosters.size(); i++) {
        if (fabs(vector_boosters[i].x - x) < PROCESS_RANGE && fabs(vector_boosters[i].y - y) < PROCESS_RANGE) {
            if (collision_check(return_x(), return_y(), return_w(), return_h(), vector_boosters[i].x + 8,
                                vector_boosters[i].y + 8, BOOSTER_W - 16, BOOSTER_H - 16)) {
    ...
    for (int i = 0; i < vector_traps.size(); i++) {
        if (fabs(vector_traps[i].x - x) < PROCESS_RANGE && fabs(vector_traps[i].y - y) < PROCESS_RANGE) {
            if (vector_traps[i].dangerous) {
                if ((!vector_traps[i].is_sharp() || (vector_traps[i].is_sharp() && !suit_sharp)) && collision_check(x,
                                                                                                                    y,
                                                                                                                    w,
                                                                                                                    h,
                                                                                                                    vector_traps
                                                                                                                    ...
                                                                                                                    return_danger_h()))
                {
                    ...
                    if (vector_traps[i].solid) {
                        handle_collision_solid(vector_traps[i].return_x(), vector_traps[i].return_y(),
                                               vector_traps[i].return_w(), vector_traps[i].return_h(), solidity,
                                               NPC_END);
                    }
                }
            }
...
bool Actor::push_actor_up (Actor* actor, double actor_x, double actor_w, bool being_pushed_up, short passed_type) {
    ...
    if (moving_upwards && actor->air_velocity >= 0) {
        ...
        // We are checking only the bottom row of the pixels for the npc against only the top row of the pixels for this
        // npc.
        if (collision_check(x, y, w, h / 2, actor_x, actor->y + actor->h / 2, actor_w, actor->h / 2)) {
            // We set the npc's y value to the y value of this npc - the npc's height, so the npc will now be right on
            // top of this npc.
            actor->y = y - actor->h;
...
void Actor::pushed_into_ceiling () {
    ...
    for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
        for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
            ...
            if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                int_y <= (level.level_y / TILE_SIZE) - 1) {
                if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID && collision_check(x, y, w, h / 2,
                                                                                                      level.tile_array[
                                                                                                          int_x][int_y].
                                                                                                      x,
                                                                                                      level.tile_array[
...
void Actor::handle_collision_solid (double passed_x, double passed_y, double passed_w, double passed_h, short solidity,
                                    short passed_type) {
    ///double check_x=1.0;
    ///double check_w=2.0;
    double check_x = 0.0;
    ...
    if (solidity == TILE_SOLIDITY_SOLID) {
        ...
        // xxxxxx
        // |----|
        if (collision_check(x - check_x, y + (h / 4), w + check_w, h / 2 + h / 8, passed_x, passed_y, passed_w,
                            passed_h)) {
            // If the left of the actor is left of the tile's left side, we know the actor is hitting the left side of
            // the tile.
            if (x < passed_x) {
                // We set the actor's x value to the x value of the tile - the actor's width, so the actor will now be
                // on the left side of the tile.
                x = passed_x - w - check_x;
            }
            // If the right side of the actor is right of the tile's right side, we know the actor is hitting the right
            // side of the tile.
        ...
        // Now we check to see if there is a y-axis collision. The section of the actor that collides with the solid
        // tile matters.
        ...
        else if (((IN_AIR && air_velocity < 0) || SWIMMING || FLYING || moving_platform_y != 0.0) &&
                 collision_check(x + 4, y, w - 8, h, passed_x, passed_y + passed_h / 2, passed_w, passed_h / 2)) {
            // We set the actor's y value to the bottom of the tile.
            y = passed_y + passed_h;
            // We set the air velocity to 0, so that the actor immediately begins falling once they bump their head on a
            // tile.
            air_velocity = 0;
        ...
        // |----|
        // xxxxxx
        // We only do this if the actor is falling, so that they don't get warped to the top of the tile if jumping into
        // the side of it.
        else if (!SWIMMING && air_velocity >= 0 && collision_check(x + 4, y + h - (h / 12), w - 8, h / 12, passed_x,
                                                                   passed_y, passed_w, passed_h / 12)) {
            // We set the actor's y value to the y value of the tile - the actor's height, so the actor will now be
            // right on top of the tile.
            y = passed_y - h;
            // We set IN_AIR to false, so that if the actor was in the air, they will no longer be.
            IN_AIR = false;
        } else if (SWIMMING && collision_check(x, y + h - (h / 4), w, (h / 4) + 1, passed_x, passed_y, passed_w,
                                               passed_h)) {
            // We set the actor's y value to the y value of the tile - the actor's height, so the actor will now be
            // right on top of the tile.
            y = passed_y - h - 1;
        ...
        if (air_velocity >= 0) {
            ...
            // This prevents a falling actor from warping back to the top of this tile, which would be caused by the
            // below if() statement alone.
            ...
            // |----|
            // xxxxxx
            // As shown above, we are checking only the bottom row of the pixels for the actor against only the top row
            // of the pixels for the tile.
            else if (collision_check(x + 4, y + h - (h / 12), w - 8, h / 12, passed_x, passed_y, passed_w,
                                     passed_h / 12)) {
                // We set the actor's y value to the y value of the tile - the actor's height, so the actor will now be
                // right on top of the tile.
                y = passed_y - h;
...
void Actor::handle_collision_tiles (int int_x, int int_y, bool crouching) {
    ...
    if (!SWIMMING && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_PASSABLE) {
        if (collision_check(x, y + h, w, h, level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y,
                            TILE_SIZE, TILE_SIZE)) {
            touching_air = true;
        }
    }
    // If the tile is a non-water non-solid tile.
    else if (SWIMMING && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_PASSABLE &&
        ...
        if (!IN_AIR && collision_check(x, y + h / 4, w, h * (3 / 4), level.tile_array[int_x][int_y].x,
                                       level.tile_array[int_x][int_y].y + TILE_SIZE / 2, TILE_SIZE, TILE_SIZE / 2)) {
        ...
        if (!IN_AIR && collision_check(x, y - TILE_SIZE + 4, w, TILE_SIZE - 4, level.tile_array[int_x][int_y].x,
                                       level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
            // Since the actor has surfaced, he is not currently underwater even though he is swimming.
            underwater = false;
        }

        if (collision_check(x, y + h * (3 / 4), w, h / 4, level.tile_array[int_x][int_y].x,
                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE / 2)) {
            SWIMMING = false;
            run_speed = 0.0;
            moving_platform_x = 0.0;
    ...
    if ((!touching_sloped_ground_last_check && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID &&
         level.tile_array[int_x][int_y].slope == 180) ||
        (FLYING && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID &&
         level.tile_array[int_x][int_y].slope != 180)) {
        handle_collision_solid(level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE,
                               TILE_SOLIDITY_SOLID, NPC_END);
    }
    // If the tile is a solid, sloping tile, and the actor is not flying.
    else if (!FLYING && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID &&
             level.tile_array[int_x][int_y].slope != 180) {
        // If the actor is on a moving platform that is moving on the x-axis,
        // he should be bumped off when he hits a slope.

        /**if(moving_platform_y==0.0 && (moving_platform_x>0.0 || on_cloud) && !IN_AIR &&
           collision_check(x,y,w/2,h,level.tile_array[int_x][int_y].x,level.tile_array[int_x][int_y].y,TILE_SIZE,TILE_SIZE)){
            fall_through_cloud();
           }
           else if(moving_platform_y==0.0 && (moving_platform_x<0.0 || on_cloud) && !IN_AIR &&
              collision_check(x+w/2,y,w/2,h,level.tile_array[int_x][int_y].x,level.tile_array[int_x][int_y].y,TILE_SIZE,TILE_SIZE)){
            fall_through_cloud();
           }
           else if(moving_platform_y>0.0 && !IN_AIR &&
              collision_check(x,y+h/2,w,h/2,level.tile_array[int_x][int_y].x,level.tile_array[int_x][int_y].y,TILE_SIZE,TILE_SIZE)){
            fall_through_cloud();
           }*/
        ...
        /**else */ if (moving_platform_x == 0.0 && !on_cloud && moving_platform_y == 0.0) {
            ...
            if (x + w / 2 >= level.tile_array[int_x][int_y].x &&
                x + w / 2 < level.tile_array[int_x][int_y].x + TILE_SIZE &&
                y <= level.tile_array[int_x][int_y].y + TILE_SIZE) {
                // If the actor is moving downwards and has reached the "ground level" of the slope, or is not moving
                // downwards or in the air at all.
                if ((((air_velocity >= 0 && IN_AIR) || (moving_platform_y > 0.0)) && y >= y_goal) ||
                    (!IN_AIR && moving_platform_y == 0.0)) {
                    // Snap to the slope.
                    y = y_goal;
                    IN_AIR = false;
                    touching_ground = true;
            ...
            if ((((air_velocity >= 0 && IN_AIR) || (moving_platform_y > 0.0)) ||
                 (!IN_AIR && moving_platform_y == 0.0)) && collision_check(x, y, w, h, level.tile_array[int_x][int_y].x,
                                                                           level.tile_array[int_x][int_y].y, TILE_SIZE,
                                                                           TILE_SIZE)) {
            ...
            if (temp_x != -1 && temp_y != -1 && temp_x >= 0 && temp_x <= (level.level_x / TILE_SIZE) - 1 &&
                temp_y >= 0 && temp_y <= (level.level_y / TILE_SIZE) - 1 &&
                (level.tile_array[temp_x][temp_y].solidity == TILE_SOLIDITY_SOLID ||
                 level.tile_array[temp_x][temp_y].solidity == TILE_SOLIDITY_CLOUD) &&
    ...
    if (level.tile_array[int_x][int_y].special == TILE_SPECIAL_WATER) {
        if (is_player && !SWIMMING && fabs(run_speed) >= WATER_RUN_THRESHOLD) {
            handle_collision_solid(level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y, TILE_SIZE,
                                   TILE_SIZE, TILE_SOLIDITY_SOLID, NPC_END);
        ...
        if (collision_check(x, y, w, h / 4, level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y,
                            TILE_SIZE, TILE_SIZE)) {
    ...
    if (level.tile_array[int_x][int_y].special == TILE_SPECIAL_CLIMBABLE_TOP && IN_AIR && air_velocity >= 0) {
        ...
        // |----|
        // If there is an x-axis collision while the actor is falling, don't check for a y-axis collision this time.
        // This prevents a falling actor from warping back to the top of this tile, which would be caused by the below
        // if() statement alone.
        // This if() statement just exists to create another condition for the below if() statement.
        if (collision_check(x, y + (h / 4), w, h / 2 + h / 8, level.tile_array[int_x][int_y].x,
                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {}
        // Now, if there was no x-axis collision, we check to see if there is a y-axis collision.
        // For y-axis hit detection we check this part of the actor's rectangle:
        // |----|
        ...
        // |----|
        // xxxxxx
        // As shown above, we are checking only the bottom row of the pixels for the actor against only the top row of
        // the pixels for the tile.
        else if (collision_check(x, y + h - (h / 12), w, h / 12, level.tile_array[int_x][int_y].x,
                                 level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE / 12)) {
            // We set the actor's y value to the y value of the tile - the actor's height, so the actor will now be
            // right on top of the tile.
    ...
    if (level.tile_array[int_x][int_y].special == TILE_SPECIAL_CLIMBABLE_TOP) {
        // If the top row of this tile's pixels are one pixel below the actor, he is standing on this tile.
        if (collision_check(x, y + h, w, 1, level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y,
                            TILE_SIZE, 1)) {
            // Thus, we let the game know the actor is standing on some ground.
            touching_ground = true;
    ...
    if (level.tile_array[int_x][int_y].special == TILE_SPECIAL_CLIMBABLE && climb_jump_timer == 0 && !crouching) {
        if ((climb_state == UP || climb_state == DOWN) && collision_check(x, y, w, h / 8,
                                                                          level.tile_array[int_x][int_y].x + w / 2 - w /
                                                                          8, level.tile_array[int_x][int_y].y,
        ...
        if (climb_state == DOWN && on_climbable_top_tile && collision_check(x, y, w, h + h / 12,
                                                                            level.tile_array[int_x][int_y].x + w / 2 -
                                                                            w / 8, level.tile_array[int_x][int_y].y,
                                                                            TILE_SIZE / 4, TILE_SIZE)) {
            IN_AIR = false;
            CLIMBING = true;
            x = level.tile_array[int_x][int_y].x + 3;
...
void Actor::handle_climbing (int int_x, int int_y) {
    ...
    if (move_state == DOWN) {
        ...
        if (level.tile_array[int_x][int_y].solidity > TILE_SOLIDITY_PASSABLE) {
            if (collision_check(x, y, w, h, level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y,
                                TILE_SIZE, TILE_SIZE)) {
                y = level.tile_array[int_x][int_y].y - h;
                CLIMBING = false;
                climb_jump_timer = climb_jump_delay;
        ...
        else if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_PASSABLE &&
                 level.tile_array[int_x][int_y].special != TILE_SPECIAL_CLIMBABLE &&
                 level.tile_array[int_x][int_y].special != TILE_SPECIAL_CLIMBABLE_TOP) {
            if (collision_check(x, y, w, h / 2, level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y,
                                TILE_SIZE, TILE_SIZE)) {
                CLIMBING = false;
                climb_jump_timer = climb_jump_delay;
                IN_AIR = true;
    ...
    // If the actor is climbing upwards.
        ...
        // If the actor hits air.
                 ...
                 level.tile_array[int_x][int_y].special != TILE_SPECIAL_CLIMBABLE &&
                 level.tile_array[int_x][int_y].special != TILE_SPECIAL_CLIMBABLE_TOP) {
            if (collision_check(x, y, w, h / 4, level.tile_array[int_x][int_y].x,
                                level.tile_array[int_x][int_y].y + TILE_SIZE - TILE_SIZE / 4, TILE_SIZE,
                                TILE_SIZE / 4)) {
                y = level.tile_array[int_x][int_y].y + TILE_SIZE;
            }
        }
...
void Actor::finish_events () {
        ...
        if (!touching_sloped_ground) {
            ...
            for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
                for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
                    if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                        int_y <= (level.level_y / TILE_SIZE) - 1) {
                        if (collision_check(return_x(), return_y(), return_w(), return_h(),
                                            level.tile_array[int_x][int_y].x, level.tile_array[int_x][int_y].y,
                                            TILE_SIZE, TILE_SIZE)) {
    ...
    if (!CLIMBING && !IN_AIR && touching_air && !touching_ground) {
        ...
        // We set the air velocity to 0, and then it increments up to the maximum fall rate in move(). By setting this
        // to 0, the actor starts falling slowly, but
    ...
    if (y < 0) {
        y = 0;
        // We set the air velocity to 0, so that the actor immediately begins falling once they bump their head on the
        // top of the level.
        air_velocity = 0;
    ...
    if (is_player && which_mp_player == player.cam_focused_index() && !player.survival_complete &&
        !player.goal_crossed && player.counter_jump_mode <= 0) {
        if (level.alarm) {
...
int Actor::distance_to_ground () {
    ...
    while (true) {
        ...
        if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
            int_y <= (level.level_y / TILE_SIZE) - 1) {
            if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID ||
                level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_CLOUD ||
                level.tile_array[int_x][int_y].special == TILE_SPECIAL_CLIMBABLE_TOP ||
                level.tile_array[int_x][int_y].special == TILE_SPECIAL_WATER) {
                break;
            } else {
...
bool Actor::touching_solid_tile (int check_x_start, int check_x_end, int check_y_start, int check_y_end) {
    for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
        for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
            ...
            if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                int_y <= (level.level_y / TILE_SIZE) - 1) {
                if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID && collision_check(x, y, w, h,
                                                                                                      level.tile_array[
                                                                                                          int_x][int_y].
                                                                                                      x,
                                                                                                      level.tile_array[
...
bool Actor::notice_wall () {
    ...
    if (x - 2 < 0 && (move_state == LEFT || move_state == LEFT_UP || move_state == LEFT_DOWN)) {
        return true;
    } else if (x + w + 2 > level.level_x &&
               (move_state == RIGHT || move_state == RIGHT_UP || move_state == RIGHT_DOWN)) {
        return true;
    }
            ...
            for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
                ...
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID) {
                        ...
                        if (collision_check(x - 2, y, w, h, level.tile_array[int_x][int_y].x,
                                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                            if (level.tile_array[int_x][int_y].x < x &&
                                (move_state == LEFT || move_state == LEFT_UP || move_state == LEFT_DOWN)) {
                                return true;
                            }
                        }
                        ...
                        if (collision_check(x, y + 2, w, h, level.tile_array[int_x][int_y].x,
                                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                            if (level.tile_array[int_x][int_y].y > y &&
                                (move_state == DOWN || move_state == LEFT_DOWN || move_state == RIGHT_DOWN)) {
                                return true;
                            }
                        }
                        ...
                        if (collision_check(x, y - 2, w, h, level.tile_array[int_x][int_y].x,
                                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                            if (level.tile_array[int_x][int_y].y < y &&
                                (move_state == UP || move_state == LEFT_UP || move_state == RIGHT_UP)) {
                                return true;
                            }
                        }
                    }
        ...
        for (int i = 0; i < vector_doors.size(); i++) {
            if (fabs(vector_doors[i].x - x) < PROCESS_RANGE && fabs(vector_doors[i].y - y) < PROCESS_RANGE) {
                if (!vector_doors[i].open) {
                    ...
                    if (collision_check(x - 2, y, w, h, vector_doors[i].x, vector_doors[i].y, vector_doors[i].w,
                                        vector_doors[i].h)) {
...
bool Actor::in_open_air () {
    ...
    for (int i = 0; i < vector_doors.size(); i++) {
        if (fabs(vector_doors[i].x - x) < PROCESS_RANGE && fabs(vector_doors[i].y - y) < PROCESS_RANGE) {
            if (!vector_doors[i].open) {
                if (collision_check(x - check_x, y, w + check_w, h, vector_doors[i].x, vector_doors[i].y,
                                    vector_doors[i].w, vector_doors[i].h)) {
                    return false;
                }
            }
