...
void Booster::render (bool mirrored) {
    if (x >= player.camera_x - BOOSTER_W && x <= player.camera_x + player.camera_w &&
        y >= player.camera_y - BOOSTER_H && y <= player.camera_y + player.camera_h) {
        double render_x = x;
        double render_y = y;
        ...
        render_sprite((int) ((int) render_x - (int) player.camera_x), (int) ((int) render_y - (int) player.camera_y),
                      image.sprite_sheet_booster, &sprites_booster[frame], 1.0, 1.0, 1.0, 0.0, COLOR_WHITE, flip);
    }
}
