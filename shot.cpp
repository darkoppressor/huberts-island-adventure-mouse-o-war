        ...
        for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
            for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if (tile->solidity == TILE_SOLIDITY_SOLID && collision_check(x, y, w, h, tile->x, tile->y,
                                                                                 TILE_SIZE, TILE_SIZE)) {
                        touching_solid = true;
                    ...
                    for (int i = 0; i < vector_npcs.size(); i++) {
                        if (fabs(vector_npcs[i].x - x) < home_range && fabs(vector_npcs[i].y - y) < home_range) {
                            if (vector_npcs[i].exists && !vector_npcs[i].counts_as_trap &&
                                !vector_npcs[i].invulnerable && !vector_npcs[i].friendly) {
                                ...
                                if (!player_shot || !player.get_upgrade_state("player_shot_noclip")) {
                                    while (true) {
                                        if (shot_x >= 0 && shot_x <= (level.level_x / TILE_SIZE) - 1 && shot_y >= 0 &&
                                            shot_y <= (level.level_y / TILE_SIZE) - 1) {
                                        ...
                                        if (shot_x == npc_x && shot_y == npc_y) {
                                            if (shot_x >= 0 && shot_x <= (level.level_x / TILE_SIZE) - 1 &&
                                                shot_y >= 0 && shot_y <= (level.level_y / TILE_SIZE) - 1) {
                                                Tile* tile = &level.tile_array[shot_x][shot_y];
...
void Shot::handle_events () {
    ...
    if (!dissipating) {
        ...
        if (kills_player && collision_check(return_x(), return_y(), return_w(), return_h(), player.return_x(),
                                            player.return_y(), player.return_w(), player.return_h())) {
            if (does_damage && (type != SHOT_FRUIT || (type == SHOT_FRUIT && !player.suit_banana)) &&
                !player.bubble_mode) {
        ...
        for (int i = 0; i < mp_players.size(); i++) {
            if (kills_player && collision_check(return_x(), return_y(), return_w(), return_h(),
                                                mp_players[i].return_x(), mp_players[i].return_y(),
                                                mp_players[i].return_w(), mp_players[i].return_h())) {
                if (does_damage && (type != SHOT_FRUIT || (type == SHOT_FRUIT && !mp_players[i].suit_banana)) &&
                    !mp_players[i].bubble_mode) {
                    // The player dies!
        ...
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                if (vector_npcs[i].exists && !vector_npcs[i].counts_as_trap) {
                    ...
                    if (kills_npc && collision_check(return_x(), return_y(), return_w(), return_h(),
                                                     vector_npcs[i].return_x(), vector_npcs[i].return_y(),
                                                     vector_npcs[i].return_w(), vector_npcs[i].return_h())) {
                        if (does_damage) {
        ...
        if (!phasing) {
                    ...
                    if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                        ...
                        if (tile->solidity == TILE_SOLIDITY_SOLID ||
                            (tile->special == TILE_SPECIAL_WATER &&
                             (!player.deadly_water || (player.deadly_water && !deadly_water_proof)))) {
                            ...
                            // xxxxxx
                            // If the tile is flat.
                            if (level.tile_array[int_x][int_y].slope == 180 && collision_check(return_x(), return_y(),
                                                                                               return_w(), return_h(),
                                ...
                                if (!player.deadly_water && player_shot &&
                            ...
                            // If the tile is sloped and the shot is moving right.
                                     ...
                                     collision_check(return_x(), return_y(), return_w() / 2.0, return_h() / 2.0,
                                                     level.tile_array[int_x][int_y].x + TILE_SIZE / 2,
                                                     level.tile_array[int_x][int_y].y + 6, TILE_SIZE / 2,
                                                     TILE_SIZE - 12) &&
                            ...
                            // If the tile is sloped and the shot is moving left.
                                     ...
                                     collision_check(return_x() + return_w() / 2.0, return_y(), return_w() / 2.0,
                                                     return_h() / 2.0, level.tile_array[int_x][int_y].x,
                                                     level.tile_array[int_x][int_y].y + 6, TILE_SIZE / 2,
                                                     TILE_SIZE - 12) && x > level.tile_array[int_x][int_y].x) {
                                // If the tile is a water tile and this is a player shot.
                                if (!player.deadly_water && player_shot &&
                                    level.tile_array[int_x][int_y].special == TILE_SPECIAL_WATER) {
                                    player.stat_water_shot++;
                            ...
                            // If the tile is sloped and the shot is moving down.
                                     ...
                                     collision_check(return_x(), return_y(), return_w(), return_h() / 2.0,
                                                     level.tile_array[int_x][int_y].x,
                                                     level.tile_array[int_x][int_y].y + TILE_SIZE / 2, TILE_SIZE,
                                                     TILE_SIZE / 2)) {
                                // If the tile is a water tile and this is a player shot.
                                if (!player.deadly_water && player_shot &&
                                    level.tile_array[int_x][int_y].special == TILE_SPECIAL_WATER) {
                        ...
                        else if (type != SHOT_FRUIT && !player_shot &&
            ...
            for (int i = 0; i < vector_traps.size(); i++) {
                if (fabs(vector_traps[i].x - x) < PROCESS_RANGE && fabs(vector_traps[i].y - y) < PROCESS_RANGE) {
                    if (vector_traps[i].dangerous && vector_traps[i].solid && collision_check(return_x(), return_y(),
                                                                                              return_w(), return_h(),
                                                                                              vector_traps[i].x,
                                                                                              vector_traps[i].y,
                                                                                              vector_traps[i].w,
            ...
            for (int i = 0; i < vector_moving_platforms.size(); i++) {
                if (fabs(vector_moving_platforms[i].x - x) < PROCESS_RANGE &&
                    ...
                    if (collision_check(return_x(), return_y(), return_w(), return_h(), vector_moving_platforms[i].x,
                                        vector_moving_platforms[i].y, vector_moving_platforms[i].w,
                                        vector_moving_platforms[i].h) && (angle > 180 && angle < 360)) {
                        if (!explosion) {
                            dissipate();
        ...
        if (!trigger_used) {
            for (int i = 0; i < vector_triggers.size(); i++) {
                if (fabs(vector_triggers[i].x - x) < PROCESS_RANGE && fabs(vector_triggers[i].y - y) < PROCESS_RANGE) {
                    if (vector_triggers[i].render_trigger && collision_check(x, y, w, h, vector_triggers[i].x,
                                                                             vector_triggers[i].y, vector_triggers[i].w,
                                                                             vector_triggers[i].h)) {
        ...
        if (!phasing) {
            ...
            for (int i = 0; i < vector_doors.size(); i++) {
                if (fabs(vector_doors[i].x - x) < PROCESS_RANGE && fabs(vector_doors[i].y - y) < PROCESS_RANGE) {
                    ...
                    if (!vector_doors[i].open && collision_check(return_x(), return_y(), return_w(), return_h(),
                                                                 vector_doors[i].x + 8, vector_doors[i].y,
                                                                 vector_doors[i].w - 16, vector_doors[i].h)) {
                        if (!explosion) {
                            dissipate();
            ...
            for (int i = 0; i < vector_shots.size(); i++) {
                if (fabs(vector_shots[i].x - x) < PROCESS_RANGE && fabs(vector_shots[i].y - y) < PROCESS_RANGE) {
                    ...
                    if (&vector_shots[i] != this && !vector_shots[i].dissipating && !vector_shots[i].get_phasing() &&
                        collision_check(x, y, w, h, vector_shots[i].x, vector_shots[i].y, vector_shots[i].w,
                                        vector_shots[i].h)) {                                                            ///collision_check(return_x(),return_y(),return_w(),return_h(),vector_shots[i].return_x(),vector_shots[i].return_y(),vector_shots[i].return_w(),vector_shots[i].return_h())
...
void Shot::dissipate () {
    if (!dissipating) {
        ...
        if (type == SHOT_PLAYER) {
            ...
            new_shots.push_back(new_shot_data((x + w / 2.0) - SHOT_SPLOSION_W / 2.0,
                                              (y + h / 2.0) - SHOT_SPLOSION_H / 2.0, 0, SHOT_PLAYER_SPLOSION, false));
        } else if (type == SHOT_PLAYER_SPLOSION) {
            // Play the shot impact sound.
            ///Probably not needed.
            ///play_positional_sound(sound_system.player_shot_impact,x,y);
        ...
        if (x >= player.camera_x - w && x <= player.camera_x + player.camera_w && y >= player.camera_y - h &&
            ...
            if (render_rotated) {
                render_sprite((int) ((int) render_x - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_shot,
                              &vector_sprites[frame], 1.0, 1.0, 1.0, render_angle);
            } else {
                render_sprite((int) ((int) render_x - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_shot,
                              &vector_sprites[frame]);
            }
