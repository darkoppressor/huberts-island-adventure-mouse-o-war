...
Trap::Trap (double get_x, double get_y, short get_type, bool get_active, short get_identifier, short get_move_state,
            double get_move_speed, double get_circle_radius) {
    x = get_x;
    y = get_y;
...
bool Trap::is_sharp () {
    if (type == TRAP_BUZZSAW || type == TRAP_ICICLE || type == TRAP_SPIKES || type == TRAP_SPIKES_DOWN ||
        type == TRAP_SPIKES_LEFT || type == TRAP_SPIKES_RIGHT || type == TRAP_SPIKES_UP) {
        return true;
    } else {
        return false;
    }
...
void Trap::render (bool mirrored) {
    ...
    if (!invisible && x >= player.camera_x - w && x <= player.camera_x + player.camera_w && y >= player.camera_y - h &&
        ...
        if (active && !moving && TRAP_IDLE_ACTIVE_SPRITES > 0) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_trap,
                          &vector_sprites_idle_active[frame_idle_active]);
        }
        // If the trap is moving and active.
        else if (active && moving && TRAP_MOVE_SPRITES > 0) {
            render_sprite((int) ((int) render_x - (int) player.camera_x),
                          (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_trap,
                          &vector_sprites_move[frame]);
        }
        // If the trap is idle and inactive.
            ...
            if (TRAP_IDLE_INACTIVE_SPRITES > 0) {
                render_sprite((int) ((int) render_x - (int) player.camera_x),
                              (int) ((int) render_y - (int) player.camera_y), *sprite_sheet_image_trap,
                              &vector_sprites_idle_inactive[frame_idle_inactive]);
            }
        }
