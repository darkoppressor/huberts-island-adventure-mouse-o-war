                ...
                if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    ...
                    if ((level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID ||
                         (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_CLOUD &&
                          !command_state(COMMAND_JUMP))) && level.tile_array[int_x][int_y].slope == 180 &&
                        !touching_sloped_ground_last_check) {
                        // First, we check for an x-axis collision. For x-axis collision, we are checking this part of
                        // the player's rectangle:
                        // |----|
                        // xxxxxx
                        // xxxxxx
                        // |----|
                        if (collision_check(x, y + 5, w, h - 10, level.tile_array[int_x][int_y].x,
                                            level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                            // If the left of the player is left of the tile's left side, we know the player is hitting
                            // the left side of the tile.
                            if (x < level.tile_array[int_x][int_y].x) {
                                // We set the player's x value to the x value of the tile - the player's width, so the
                                // player will now be on the left side of the tile.
                                x = level.tile_array[int_x][int_y].x - w;
                            }
                            // If the right side of the player is right of the tile's right side, we know the player is
                        ...
                        // Next, we check for a y-axis collision. For y-axis collision, we are checking this part of the
                        // player's rectangle:
                        // |-xx-|
                        // |-xx-|
                        // |-xx-|
                        // |-xx-|
                        else if (collision_check(x + 5, y, w - 10, h, level.tile_array[int_x][int_y].x,
                                                 level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                            // If the top of the player is above the tile's top side, we know the player is hitting the
                            // top side of the tile.
                            if (y < level.tile_array[int_x][int_y].y) {
                                // We set the player's y value to the y value of the tile - the player's height, so the
                                // player will now be on the top side of the tile.
                                y = level.tile_array[int_x][int_y].y - h;
                            }
                            // If the bottom side of the player is below the tile's bottom side, we know the player is
                            // hitting the bottom side of the tile.
                    ...
                    // If the tile is a solid, sloping tile.
                    else if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID &&
                        ...
                        if (collision_check(x, y, w, h, level.tile_array[int_x][int_y].x,
                            ...
                            if (level.tile_array[int_x][int_y].slope == 45) {
                                ...
                                difference_top = level.tile_array[int_x][int_y].y + TILE_SIZE - h - TILE_SIZE +
                                                 x_position;
                                difference_bottom = level.tile_array[int_x][int_y].y + TILE_SIZE - TILE_SIZE +
                                                    x_position;
                                difference_top = y - difference_top;
                                difference_bottom = y - difference_bottom;
                            ...
                            if (x + w / 2 >= level.tile_array[int_x][int_y].x &&
                                ...
                                if ((direction == DOWN && move_state != UP && move_state != RIGHT_UP &&
                                     move_state != LEFT_UP && move_state != NONE) ||
                                    (direction == UP && move_state != DOWN && move_state != RIGHT_DOWN &&
                                     move_state != LEFT_DOWN && move_state != NONE)) {
                                    if (level.tile_array[int_x][int_y].slope == 45) {
                                        if (direction == DOWN &&
                                        ...
                                        if (direction == DOWN &&
                                            y >=
                                            level.tile_array[int_x][int_y].y + TILE_SIZE - h - TILE_SIZE + x_position) {
                                            // Snap to the slope.
                                            y = level.tile_array[int_x][int_y].y + TILE_SIZE - h - TILE_SIZE +
                                                x_position;
                                            touching_sloped_ground = true;
                                        } else if (direction == UP &&
                                                   y <=
            ...
            if (vector_items[i].type <= 0) {
                if (collision_check(x, y, w, h, vector_items[i].x, vector_items[i].y, vector_items[i].w,
                    ...
                    if (!tooltip_setup) {
                        ...
                        tooltip_levelname.setup(tooltip_text, (camera_w - tooltip_text.length() * font.spacing_x) / 2.0,
                                                48);
            ...
            if (vector_items[i].exists) {
                if (vector_items[i].type <= 0) {
                    if (collision_check(x - TILE_SIZE * 4, y - TILE_SIZE * 4, w + TILE_SIZE * 8, h + TILE_SIZE * 8,
                                        vector_items[i].x, vector_items[i].y, vector_items[i].w, vector_items[i].h)) {
                        worldmap_region = level.return_level_region(vector_items[i].type);
                        break;
                    }
                }
...
void Player::worldmap_render () {
    if (move_state == 0 && (facing == LEFT || facing == LEFT_UP || facing == LEFT_DOWN)) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_idle_left[frame_idle]);
    } else if (move_state == 0 && (facing == RIGHT || facing == RIGHT_UP || facing == RIGHT_DOWN)) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_idle_right[frame_idle]);
    } else if (move_state == 0 && facing == UP) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_idle_up[frame_idle]);
    } else if (move_state == 0 && facing == DOWN) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_idle_down[frame_idle]);
    } else if (move_state != 0 && (facing == LEFT || facing == LEFT_UP || facing == LEFT_DOWN)) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_walk_left[frame]);
    } else if (move_state != 0 && (facing == RIGHT || facing == RIGHT_UP || facing == RIGHT_DOWN)) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_walk_right[frame]);
    } else if (move_state != 0 && facing == UP) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_walk_up[frame]);
    } else if (move_state != 0 && facing == DOWN) {
        render_sprite((int) ((int) x - (int) camera_x), (int) ((int) y - (int) camera_y), *ptr_player_worldmap_image,
                      &sprites_player_worldmap_walk_down[frame]);
    }
