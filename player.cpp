...
void Player::reset () {
    ...
    for (int i = 0; i <= LAST_LEVEL; i++) {
        ...
        if (i == 5) {
            ...
            boss_messages[boss_messages.size() -
                          1].message =
                "Off in the distance, towards the keep, came a roar!\nHubert was startled, but ready for more!";
        } else if (i == 7) {
            boss_messages[boss_messages.size() - 1].message_title = "Boss unlocked: " + level.return_level_name(i);
            boss_messages[boss_messages.size() -
                          1].message =
                "From the direction of the old haunted manor\nCame quite the moan, in a very loud manner!";
        } else if (i == 10) {
            boss_messages[boss_messages.size() - 1].message_title = "Boss unlocked: " + level.return_level_name(i);
            boss_messages[boss_messages.size() -
                          1].message =
                "A great clash arose from the mountain of fire!\nHubert's situation grows ever more dire!";
        } else if (i == 12) {
            boss_messages[boss_messages.size() - 1].message_title = "Boss unlocked: " + level.return_level_name(i);
            boss_messages[boss_messages.size() -
                          1].message =
                "From the old tomb came a scream and a rumble!\nHubert set off, without even a grumble.";
        } else if (i == 14) {
            boss_messages[boss_messages.size() - 1].message_title = "Boss unlocked: " + level.return_level_name(i);
            boss_messages[boss_messages.size() -
                          1].message =
                "At last! His lieutenants defeated and smashed,\nThe pirate-king Dakkar is prepared for a clash!";
        }
    }
...
void Player::default_keys_keyboard () {
    // Note that commands must be setup here in the same order as command identifiers are setup in the enum in
    // enumerations.h.
    keys.clear();
...
void Player::start_game () {
    ...
    if (!game_in_progress) {
        ...
        if (game_mode == GAME_MODE_SP_ADVENTURE) {} else if (game_mode == GAME_MODE_MP_ADVENTURE) {
            current_level = 0;
            current_sub_level = 0;
...
void Player::beat_level () {
        ...
        if (main_levels_beaten % 2 == 0 && return_main_bosses_unlocked() < 4) {
            ...
            while (true) {
                ...
                if (new_boss_level == -1) {
                    ...
                    // Play story message.
                    window_message[0].set_message(boss_messages[new_boss_level].message_title,
                                                  boss_messages[new_boss_level].message);
    ...
    if (current_level == 14) {
        ...
        if (bosses[14] == BOSS_STATE_DEFEATED && achievements[ACHIEVEMENT_COLLECT_ALL_LEAVES] &&
            achievements[ACHIEVEMENT_COLLECT_ALL_CHEESE]) {
            msg +=
                "Perhaps this is finally the end of our tale!\nMighty Hubert retired to rest in the dale.\nWith his main squeeze Sally, stacks of leaves and cheese,\nHubert was happy, content as could be!";
            msg_title = "The End";
        } else {
...
void Player::move () {
    ...
    if (!bubble_mode) {
        ...
        // complete the death.
        if (!game_mode_is_multiplayer() && DYING && !collision_check(x, y, w, h, camera_x, camera_y, camera_w,
                                                                     camera_h)) {
            // If the goal has already been crossed, the player gets credit for beating
            // the level even if he dies.
            if (goal_crossed) {
        ...
        if (game_mode_is_multiplayer()) {
            if (!DYING && which_mp_player != cam_focused_index() && !collision_check(x, y, w, h, camera_x, camera_y,
                                                                                     camera_w, camera_h)) {
                put_in_bubble();
            ...
            if (!hint) {
                for (int i = 0; i < vector_triggers.size(); i++) {
                    if (fabs(vector_triggers[i].x - x) < PROCESS_RANGE &&
                        ...
                        if (!vector_triggers[i].trigger_method && collision_check(x, y, w, h, vector_triggers[i].x,
                                                                                  vector_triggers[i].y,
                                                                                  vector_triggers[i].w,
                                                                                  vector_triggers[i].h)) {
                            if (vector_triggers[i].render_trigger != 0) {
                                ...
                                tooltip_hint.setup(tooltip_text,
                                                   (camera_w - tooltip_text.length() * font.spacing_x) / 2.0, 84);
    ...
    if (!bubble_mode) {
        ...
        if (!DYING) {
            ...
            if (!CLIMBING) {
                ...
                if (IN_AIR) {
                    ...
                    // As long as air_velocity hasn't exceeded gravity_max, the maximum speed an object can fall, add
                    // gravity to air_velocity.
                ...
                if (SWIMMING && move_state != UP && move_state != LEFT_UP && move_state != RIGHT_UP &&
                    move_state != DOWN && move_state != LEFT_DOWN && move_state != RIGHT_DOWN) {
                    // Float upwards.
                ...
                if (SWIMMING && move_state != LEFT && move_state != LEFT_DOWN && move_state != LEFT_UP &&
                    move_state != RIGHT && move_state != RIGHT_DOWN && move_state != RIGHT_UP) {
                    if (run_speed > 0.0) {
        ...
        // If the player is dying.
            ...
            if (collision_check(x, y, w, h, 0, 0, level.level_x, level.level_y)) {
                ...
                // As long as air_velocity hasn't exceeded jump_max, the maximum speed an object can fall, add gravity
                // to air_velocity.
                if (air_velocity < gravity_max) {
                    air_velocity += gravity;
                }
    ...
    // If in bubble mode.
        ...
        if (!cam_focused_dying() && collision_check(x + w / 4.0, y + h / 8.0, w / 2.0, h / 4.0,
                                                    cam_focused_x() + cam_focused_w() / 4.0,
                                                    cam_focused_y() + cam_focused_h() / 8.0, cam_focused_w() / 2.0,
                                                    cam_focused_h() / 4.0)) {
            bubble_mode = false;
...
void Player::handle_tiles (int check_x_start, int check_x_end, int check_y_start, int check_y_end) {
    for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
        for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
            ...
            if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                int_y <= (level.level_y / TILE_SIZE) - 1) {
...
void Player::handle_tile_hazard (int int_x, int int_y) {
    if (level.tile_array[int_x][int_y].special == TILE_SPECIAL_HAZARD) {
        if (collision_check(return_x(), return_y(), return_w(), return_h(), level.tile_array[int_x][int_y].return_x(),
                            level.tile_array[int_x][int_y].return_y(), TILE_SIZE, TILE_SIZE)) {
            // If the player was not already dying, and is not invulnerable.
            if (!DYING && !invulnerable) {
                stat_deaths_traps++;
            }

            handle_death(level.tile_array[int_x][int_y].return_x(), level.tile_array[int_x][int_y].return_y(),
                         TILE_SIZE, TILE_SIZE);
        }
    }
    // If the tile is a water tile and water tiles are deadly.
    else if (deadly_water && !suit_deadly_water && SWIMMING &&
             level.tile_array[int_x][int_y].special == TILE_SPECIAL_WATER) {
        if (collision_check(return_x(), return_y(), return_w(), return_h(), level.tile_array[int_x][int_y].return_x(),
                            level.tile_array[int_x][int_y].return_y(), TILE_SIZE, TILE_SIZE)) {
            // If the player was not already dying, and is not invulnerable.
            if (!DYING && !invulnerable) {
...
void Player::handle_events (bool being_pushed_up) {
    ...
    if (!DYING) {
        ...
        for (int i = 0; i < vector_items.size(); i++) {
            if (fabs(vector_items[i].x - x) < PROCESS_RANGE && fabs(vector_items[i].y - y) < PROCESS_RANGE) {
                ...
                if (vector_items[i].exists) {
                    if (collision_check(x, y, w, h, vector_items[i].x, vector_items[i].y, vector_items[i].w,
                                        vector_items[i].h)) {
                        if (!(vector_items[i].type == ITEM_SPAWNPOINT || vector_items[i].type == ITEM_CHECKPOINT ||
                              vector_items[i].type == ITEM_ENDPOINT)) {
                        ...
                        if (vector_items[i].type == ITEM_LEAF) {
                            ...
                            gain_score(SCORES_ITEMS[ITEM_LEAF], vector_items[i].x + vector_items[i].w / 2.0,
                                       vector_items[i].y);
                            ...
                            gain_score(SCORES_ITEMS[ITEM_CHEESE], vector_items[i].x + vector_items[i].w / 2.0,
                                       vector_items[i].y);
                            ...
                            gain_score(SCORES_ITEMS[ITEM_J_BALLOON], vector_items[i].x + vector_items[i].w / 2.0,
                                       vector_items[i].y);
                            ...
                            if (game_mode_is_multiplayer()) {
                                int player_count = 1 + mp_players.size();
                                int ammo_per_player = floor((double) return_ammo_barrel_amount() /
                                                            (double) player_count);
                            ...
                            gain_score(return_candy_score(vector_items[i].score_bonus),
                                       vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                            ...
                            if (next_available_slot != -1) {
                                ...
                                if (vector_items[i].type == ITEM_SWIMMING_GEAR) {
                                    ...
                                    gain_score(SCORES_ITEMS[ITEM_SWIMMING_GEAR],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_RED) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_RED], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_BLUE) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_BLUE], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_GREEN) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_GREEN],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_YELLOW) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_YELLOW],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_ORANGE) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_ORANGE],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_PURPLE) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_PURPLE],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_TOWEL) {
                                    gain_score(SCORES_ITEMS[ITEM_TOWEL], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_GRAY) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_GRAY], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_BROWN) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_BROWN],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_BLACK) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_BLACK],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_PINK) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_PINK], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_KEY_CYAN) {
                                    gain_score(SCORES_ITEMS[ITEM_KEY_CYAN], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SINK) {
                                    profile.update_achievements(ACHIEVEMENT_JOKE_ITEM);

                                    gain_score(SCORES_ITEMS[ITEM_SINK], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SUIT_DEADLY_WATER) {
                                    new_special_item = true;

                                    gain_score(SCORES_ITEMS[ITEM_SUIT_DEADLY_WATER],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SUIT_SHARP) {
                                    new_special_item = true;

                                    gain_score(SCORES_ITEMS[ITEM_SUIT_SHARP],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SUIT_BANANA) {
                                    new_special_item = true;

                                    gain_score(SCORES_ITEMS[ITEM_SUIT_BANANA],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_SHOT_HOMING) {
                                    new_special_item = true;

                                    gain_score(SCORES_ITEMS[ITEM_SHOT_HOMING],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_TRANSLATOR) {
                                    new_special_item = true;

                                    gain_score(SCORES_ITEMS[ITEM_TRANSLATOR],
                                               vector_items[i].x + vector_items[i].w / 2.0, vector_items[i].y);
                                } else if (vector_items[i].type == ITEM_J_WING) {
                                    new_special_item = true;

                                    gain_score(SCORES_ITEMS[ITEM_J_WING], vector_items[i].x + vector_items[i].w / 2.0,
                                               vector_items[i].y);
                                }
                                ...
                                if (vector_items[i].type == ITEM_SWIMMING_GEAR) {
                                    ...
                                    play_positional_sound(sound_system.item_collect_swimming_gear, x, y);
                                            ...
                                            vector_items[i].type <= ITEM_KEY_PURPLE) ||
                                           (vector_items[i].type >= ITEM_KEY_GRAY &&
                                            vector_items[i].type <= ITEM_KEY_CYAN)) {
                    ...
                    if (vector_items[i].type == ITEM_SPAWNPOINT) {
                        if (collision_check(x + w / 4.0, y, w - w / 2.0, h, vector_items[i].x + 16,
                                            vector_items[i].y + 16, 1, 16)) {
                    ...
                    if (vector_items[i].type == ITEM_CHECKPOINT) {
                        if (collision_check(x, y, w, h, vector_items[i].x, vector_items[i].y - 64 * 2, 32,
                                            96 + 64 * 2)) {
                ...
                if (vector_npcs[i].exists) {
                    if (!CLIMBING && !vector_npcs[i].CLIMBING) {
                        if (!cheat_noclip) {
                            ...
                            handle_collision_solid(vector_npcs[i].x, vector_npcs[i].y, vector_npcs[i].w,
                                                   vector_npcs[i].h, solidity, vector_npcs[i].type);
                    ...
                    if (!SLIDING &&
                        ((!vector_npcs[i].special_attack_in_progress && vector_npcs[i].deadly_to_touch) ||
                         (vector_npcs[i].special_attack_in_progress &&
                          vector_npcs[i].deadly_while_special_attacking)) && collision_check(return_x(), return_y(),
                                                                                             return_w(), return_h(),
                                                                                             vector_npcs[i].return_x(),
                                                                                             vector_npcs[i].return_y(),
                                                                                             vector_npcs[i].return_w(),
                                                                                             vector_npcs[i].return_h()))
                    {
                        // If the player was not already dying, and is not invulnerable.
                        if (!DYING && !invulnerable) {
                        ...
                        // The player dies!
                        handle_death(vector_npcs[i].return_x(), vector_npcs[i].return_y(), vector_npcs[i].return_w(),
                                     vector_npcs[i].return_h());
                    } else if (SLIDING && !vector_npcs[i].counts_as_trap && collision_check(return_x(), return_y(),
                                                                                            return_w(), return_h(),
                                                                                            vector_npcs[i].return_x(),
                                                                                            vector_npcs[i].return_y(),
                                                                                            vector_npcs[i].return_w(),
        ...
        // ******************************************//
        for (int i = 0; i < mp_players.size(); i++) {
            if (!CLIMBING && !CROUCHING && !mp_players[i].DYING && !mp_players[i].bubble_mode &&
                !mp_players[i].CLIMBING && !mp_players[i].CROUCHING) {
                if (!cheat_noclip) {
                    short solidity;
                    ...
                    handle_collision_solid(mp_players[i].return_x(), mp_players[i].y, mp_players[i].return_w(),
                                           mp_players[i].h, solidity, NPC_END);

                    ///handle_player_platform(i);

                    if (!mp_players[i].SWIMMING) {
                        if (push_actor_up(&mp_players[i], mp_players[i].return_x(), mp_players[i].return_w(),
                                          being_pushed_up, NPC_END)) {
    ...
    // If the player is dying.
        ...
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                ...
                if (vector_npcs[i].exists) {
                    ...
                    if (((!vector_npcs[i].special_attack_in_progress && vector_npcs[i].deadly_to_touch) ||
                         (vector_npcs[i].special_attack_in_progress &&
                          vector_npcs[i].deadly_while_special_attacking)) && collision_check(return_x(), return_y(),
                                                                                             return_w(), return_h(),
                                                                                             vector_npcs[i].return_x(),
        ...
        if (y < 0) {
            y = 0;
            // We set the air velocity to 0, so that the actor immediately begins falling once they bump their head on
...
void Player::handle_death (double death_x, double death_y, double death_w, double death_h,
                           bool override_invulnerability) {
...
void Player::render_menu_pretties () {
    ...
    if (!game_in_progress) {
        ...
        if (is_night()) {
            render_rectangle(main_window.SCREEN_WIDTH - sprites_moon[moon_phase].w - 15 - 15, 0,
                             sprites_moon[moon_phase].w + 30, sprites_moon[moon_phase].h + 30, 1.0, COLOR_BLUE_OCEAN);
            render_rectangle(main_window.SCREEN_WIDTH - sprites_moon[moon_phase].w - 15 - 13, 2,
                             sprites_moon[moon_phase].w + 26, sprites_moon[moon_phase].h + 26, opacity, COLOR_BLACK);
            render_sprite(main_window.SCREEN_WIDTH - sprites_moon[moon_phase].w - 15, 15, image.moon,
                          &sprites_moon[moon_phase]);
        } else {
            render_rectangle(main_window.SCREEN_WIDTH - sprites_moon[moon_phase].w - 15 - 15, 0,
                             sprites_moon[moon_phase].w + 30, sprites_moon[moon_phase].h + 30, 1.0, COLOR_BLUE);
            render_rectangle(main_window.SCREEN_WIDTH - sprites_moon[moon_phase].w - 15 - 13, 2,
                             sprites_moon[moon_phase].w + 26, sprites_moon[moon_phase].h + 26, 1.0, COLOR_SKY_BLUE);
        ...
        msg = buff;
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15 + 2,
                  15 + sprites_moon[moon_phase].h + 20 + 2, msg, COLOR_BLACK);
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15, 15 + sprites_moon[moon_phase].h + 20,
                  msg, return_gui_color(holiday, 3));
        strftime(buff, sizeof buff, "%A", tm_now);
        msg = buff;
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15 + 2,
                  15 + sprites_moon[moon_phase].h + 20 + font.spacing_y + 2, msg, COLOR_BLACK);
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15,
                  15 + sprites_moon[moon_phase].h + 20 + font.spacing_y, msg, return_gui_color(holiday, 3));
        strftime(buff, sizeof buff, "%B %d", tm_now);
        msg = buff;
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15 + 2,
                  15 + sprites_moon[moon_phase].h + 20 + font.spacing_y * 2 + 2, msg, COLOR_BLACK);
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15,
                  15 + sprites_moon[moon_phase].h + 20 + font.spacing_y * 2, msg, return_gui_color(holiday, 3));
        strftime(buff, sizeof buff, "%Y", tm_now);
        msg = buff;
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15 + 2,
                  15 + sprites_moon[moon_phase].h + 20 + font.spacing_y * 3 + 2, msg, COLOR_BLACK);
        font.show(main_window.SCREEN_WIDTH - msg.length() * font.spacing_x - 15,
                  15 + sprites_moon[moon_phase].h + 20 + font.spacing_y * 3, msg, return_gui_color(holiday, 3));
    ...
    if (menu_title_font) {
        if (!game_in_progress) {
            ...
            for (int i = 0; i < colored_message.length(); i++) {
                font_large.show((main_window.SCREEN_WIDTH - colored_message.length() * font_large.spacing_x * 2.0) /
                                2.0 + (i * font_large.spacing_x * 2.0) + 4, 15 + 4, colored_message.substr(i, 1),
                                COLOR_BLACK, 1.0, 2.0);
                font_large.show((main_window.SCREEN_WIDTH - colored_message.length() * font_large.spacing_x * 2.0) /
                                2.0 + (i * font_large.spacing_x * 2.0), 15, colored_message.substr(i, 1),
                                colors[msg_color], 1.0, 2.0);
            ...
            for (int i = 0; i < colored_message.length(); i++) {
                font_large.show((main_window.SCREEN_WIDTH - colored_message.length() * font_large.spacing_x * 2.0) /
                                2.0 + (i * font_large.spacing_x * 2.0) + 4, 47 + 4, colored_message.substr(i, 1),
                                COLOR_BLACK, 1.0, 2.0);
                font_large.show((main_window.SCREEN_WIDTH - colored_message.length() * font_large.spacing_x * 2.0) /
                                2.0 + (i * font_large.spacing_x * 2.0), 47, colored_message.substr(i, 1),
                                colors[msg_color], 1.0, 2.0);
            ...
            for (int i = 0; i < colored_message.length(); i++) {
                font.show((main_window.SCREEN_WIDTH - colored_message.length() * font.spacing_x) * 0.95 +
                          (i * font.spacing_x) + 2, 543 + 2, colored_message.substr(i, 1), COLOR_BLACK);
                font.show((main_window.SCREEN_WIDTH - colored_message.length() * font.spacing_x) * 0.95 +
                          (i * font.spacing_x), 543, colored_message.substr(i, 1), colors[msg_color]);
            ...
            for (int i = 0; i < colored_message.length(); i++) {
                font_sga.show((main_window.SCREEN_WIDTH - colored_message.length() * font.spacing_x) * 0.95 +
                              (i * font.spacing_x) + 2, 575 + 2, colored_message.substr(i, 1), COLOR_BLACK);
                font_sga.show((main_window.SCREEN_WIDTH - colored_message.length() * font.spacing_x) * 0.95 +
                              (i * font.spacing_x), 575, colored_message.substr(i, 1), colors[msg_color]);
        ...
        if (new_game_plus > 0) {
            ...
            for (int i = 0; i < colored_message.length(); i++) {
                font.show((main_window.SCREEN_WIDTH - colored_message.length() * font.spacing_x) * 0.95 +
                          (i * font.spacing_x) + 2, 15 + sprites_moon[moon_phase].h + 20 + font.spacing_y * 4 + 2,
                          colored_message.substr(i, 1), COLOR_BLACK);
                font.show((main_window.SCREEN_WIDTH - colored_message.length() * font.spacing_x) * 0.95 +
                          (i * font.spacing_x), 15 + sprites_moon[moon_phase].h + 20 + font.spacing_y * 4,
                          colored_message.substr(i, 1), colors[msg_color]);
        ...
        if (!game_in_progress) {
            ...
            for (int i = 0; i < colored_message.length(); i++) {
                font_large2.show((main_window.SCREEN_WIDTH - colored_message.length() * font_large.spacing_x * 2.0) /
                                 2.0 + (i * font_large.spacing_x * 2.0) + 4, 15 + 4, colored_message.substr(i, 1),
                                 COLOR_BLACK, 1.0, 2.0);
                font_large2.show((main_window.SCREEN_WIDTH - colored_message.length() * font_large.spacing_x * 2.0) /
                                 2.0 + (i * font_large.spacing_x * 2.0), 15, colored_message.substr(i, 1),
...
void Player::render_foreground () {
    if (game_in_progress) {
        if (level.alarm) {
            render_rectangle(0, 0, main_window.SCREEN_WIDTH, main_window.SCREEN_HEIGHT, opacity_overlay_alarm,
                             COLOR_RED);
        }

        if (night_mode) {
            render_texture((int) (x - camera_x - DEFAULT_SCREEN_WIDTH + w / 2),
                           (int) (y - camera_y - DEFAULT_SCREEN_HEIGHT + h / 2), image.overlay_night);
        }
...
void Player::render_time_overlay_fancy () {
    ...
    for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
        for (int int_y = check_y_start; int_y < check_y_end; int_y++) {
            if (int_x >= 0 && int_x <= (level.level_x / LIGHTING_TILE_SIZE) - 1 && int_y >= 0 &&
                int_y <= (level.level_y / LIGHTING_TILE_SIZE) - 1) {
                ...
                if (!tile_lit[int_x - check_x_start][int_y - check_y_start]) {
                    ...
                    if (int_x < (level.level_x / LIGHTING_TILE_SIZE) - 1 &&
                        int_y < (level.level_y / LIGHTING_TILE_SIZE) - 1 && int_x < check_x_end - 1 &&
                        int_y < check_y_end - 1) {
                        for (int i = 0; i < 3; i++) {
                            int light_check_x = 0;
                            int light_check_y = 0;
                            ...
                            if (light_data[light_check_x][light_check_y].light_intensity !=
                                light_data[int_x][int_y].light_intensity) {
                                tile_square = false;
                                break;
                            }
                            ...
                            if (light_data[light_check_x][light_check_y].color.red !=
                                light_data[int_x][int_y].color.red) {
                                tile_square = false;
                                break;
                            }

                            if (light_data[light_check_x][light_check_y].color.green !=
                                light_data[int_x][int_y].color.green) {
                                tile_square = false;
                                break;
                            }

                            if (light_data[light_check_x][light_check_y].color.blue !=
                                light_data[int_x][int_y].color.blue) {
                                tile_square = false;
                                break;
                            }
                        }
                    ...
                    if (!light_data[int_x][int_y].seen) {
                        if (opacity_actual > 0.0) {
                            ...
                            render_rectangle((int) (tile_x - (int) camera_x), (int) (tile_y - (int) camera_y),
                                             LIGHTING_TILE_SIZE * dimension_mod, LIGHTING_TILE_SIZE * dimension_mod,
                                             opacity_actual, COLOR_BLACK);
                        }
                    } else {
                        ...
                        if ((light_opacity > 0.0 && dimness < opacity_actual) || dimness > 0.0) {
                            ...
                            Light_Data blended = level.blend_light(dimness, color_name_to_doubles(COLOR_BLACK),
                                                                   light_opacity, light_data[int_x][int_y].color);

                            render_rectangle((int) (tile_x - (int) camera_x), (int) (tile_y - (int) camera_y),
                                             LIGHTING_TILE_SIZE * dimension_mod, LIGHTING_TILE_SIZE * dimension_mod,
                                             blended.light_intensity, blended.color);
                        }
                    }
                }
            }
...
void Player::animate () {
    ...
    if (frame_counter >= 8 - fabs(run_speed)) {
        ...
        if (!SWIMMING && !IN_AIR && !SHOOTING && !CLIMBING && !DYING && !LOOKING && run_speed != 0.0 &&
            frame % 7 == 0) {
            play_footstep_sound();
        }
    ...
    if ((frame_counter_jump >= 20 && frame_jump < 3) || (frame_counter_jump >= 30 && frame_jump == 3) ||
        (frame_counter_jump >= 5 && frame_jump >= 4)) {
        frame_counter_jump = 0;
...
void Player::render (bool mirrored) {
    ...
    // If the player is dying.
    if (DYING) {
        render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                      *ptr_player_image, &sprites_player_death[frame_death]);
    }
    // If the player is on the ground.
    else if (!SWIMMING && !IN_AIR && !SHOOTING && !CLIMBING && !LOOKING && !CROUCHING) {
        if (run_speed == 0.0 && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_idle_left[frame_idle]);
        } else if (run_speed == 0.0 && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_idle_right[frame_idle]);
        } else if (run_speed != 0.0 && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_left[frame]);
        } else if (run_speed != 0.0 && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_right[frame]);
        }
    }
    // If the player is crouching.
        ...
        if (facing == LEFT || facing == LEFT_UP || facing == LEFT_DOWN) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_crouch_left[0]);
        } else if (facing == RIGHT || facing == RIGHT_UP || facing == RIGHT_DOWN) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_crouch_right[0]);
        }
    }
    // If the player is swimming.
    else if (SWIMMING) {
        if (move_state == 0 && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_swim_left[frame_swim]);
        } else if (move_state == 0 && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_swim_right[frame_swim]);
        } else if (move_state == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_swim_up_right[frame_swim]);
        } else if (move_state == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_swim_up_left[frame_swim]);
        } else if (move_state == LEFT || move_state == LEFT_DOWN || move_state == LEFT_UP ||
                   (move_state == DOWN && facing == LEFT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_swim_left[frame_swim]);
        } else if (move_state == RIGHT || move_state == RIGHT_DOWN || move_state == RIGHT_UP ||
                   (move_state == DOWN && facing == RIGHT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_swim_right[frame_swim]);
        }
    }
    // If the player is in the air and not shooting.
        ...
        if (facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_jump_right[frame_jump]);
        } else if (facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_jump_left[frame_jump]);
        }
    }
    // If the player is shooting and not in the air or climbing.
        ...
        if (shoot_render_direction == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_up_left[frame_shoot]);
        } else if (shoot_render_direction == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_up_right[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_down_left_air[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_down_right_air[frame_shoot]);
        } else if (shoot_render_direction == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_right[frame_shoot]);
        } else if (shoot_render_direction == LEFT) {
            render_sprite((int) ((int) render_x - 4 - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_left[frame_shoot]);
        }
    }
    // If the player is shooting and in the air.
        ...
        if (shoot_render_direction == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_up_left_air[frame_shoot]);
        } else if (shoot_render_direction == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_up_right_air[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_down_left_air[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == RIGHT) {
            ...
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_right_air[frame_shoot]);
        } else if (shoot_render_direction == LEFT) {
            render_sprite((int) ((int) render_x - 4 - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_left_air[frame_shoot]);
        }
    }
    // If the player is shooting and climbing.
        ...
        if (shoot_render_direction == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_climb_up_right[frame_shoot]);
        } else if (shoot_render_direction == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_climb_up_left[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_climb_down_right[frame_shoot]);
        } else if (shoot_render_direction == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_climb_down_left[frame_shoot]);
        } else if (shoot_render_direction == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_climb_right[frame_shoot]);
        } else if (shoot_render_direction == LEFT) {
            render_sprite((int) ((int) render_x - 4 - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_shoot_climb_left[frame_shoot]);
        }
    }
    // If the player is climbing and not shooting or looking.
        ...
        if ((move_state != UP && move_state != DOWN) && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_climb_up_right[0]);
        } else if ((move_state != UP && move_state != DOWN) && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_climb_up_left[0]);
        } else if (move_state == UP && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_climb_up_right[frame_climb]);
        } else if (move_state == UP && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_climb_up_left[frame_climb]);
        } else if (move_state == DOWN && facing == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_climb_down_right[frame_climb]);
        } else if (move_state == DOWN && facing == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_climb_down_left[frame_climb]);
        }
    }
    // If the player is looking and not climbing.
        ...
        if ((look_state == 0 && facing == RIGHT) || look_state == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_right[0]);
        } else if ((look_state == 0 && facing == LEFT) || look_state == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_left[0]);
        } else if (look_state == RIGHT_UP || (look_state == UP && facing == RIGHT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_right_up[0]);
        } else if (look_state == RIGHT_DOWN || (look_state == DOWN && facing == RIGHT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_right_down[0]);
        } else if (look_state == LEFT_UP || (look_state == UP && facing == LEFT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_left_up[0]);
        } else if (look_state == LEFT_DOWN || (look_state == DOWN && facing == LEFT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_left_down[0]);
        }
    }
    // If the player is both looking and climbing.
        ...
        if ((look_state == 0 && facing == RIGHT) || look_state == RIGHT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_right_climb_down[0]);
        } else if ((look_state == 0 && facing == LEFT) || look_state == LEFT) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_left_climb_down[0]);
        } else if (look_state == RIGHT_UP || (look_state == UP && facing == RIGHT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_right_climb_up[0]);
        } else if (look_state == RIGHT_DOWN || (look_state == DOWN && facing == RIGHT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_right_climb_down[0]);
        } else if (look_state == LEFT_UP || (look_state == UP && facing == LEFT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_left_climb_up[0]);
        } else if (look_state == LEFT_DOWN || (look_state == DOWN && facing == LEFT)) {
            render_sprite((int) ((int) render_x - (int) camera_x), (int) ((int) render_y - (int) camera_y),
                          *ptr_player_image, &sprites_player_look_left_climb_down[0]);
        }
    ...
    if (counter_jump_mode > 0 && !DYING) {
        ...
        if (counter_jump_mode < 240 && counter_jump_mode >= 120 && counter_jump_mode % 10 >= 0 &&
            counter_jump_mode % 10 <= 5) {
            render_wings = false;
        } else if (counter_jump_mode < 120 && counter_jump_mode >= 60 && counter_jump_mode % 10 >= 6 &&
                   counter_jump_mode % 10 <= 10) {
            render_wings = false;
        ...
        if (render_wings) {
            render_sprite((int) ((int) x - 8 - (int) camera_x), (int) ((int) y - (int) camera_y), image.powerup_jump,
                          &sprites_powerup_jump[frame_powerup], 1.0, 1.0, scale_y);
        }
    }
    ...
    if (bubble_mode && !DYING) {
        ...
        render_sprite((int) ((int) x - (balloon_width - w) / 2 - (int) camera_x),
                      (int) ((int) y - (balloon_height - h) / 2 - (int) camera_y), image.balloon, &balloon_rect, 1.0,
                      balloon_scale_x, balloon_scale_y, 0.0, get_player_color(which_mp_player));
    }
...
short Player::next_available_inventory_slot () {
    ...
    // Otherwise, find the next empty slot.
        ...
        for (short check_slot = 0; next_available_slot == -1 && check_slot < window_inventory[0].INVENTORY_MAX_SIZE;
             check_slot++) {
            for (int i = 0; i < inventory.size(); i++) {
...
void Player::create_firework () {
    if (vector_effect_firework.size() < option_effect_limit) {
        ...
        vector_effect_firework.push_back(Effect_Firework((double) random_range(200,
                                                                               main_window.SCREEN_WIDTH - 200) +
                                                         camera_x, (double) main_window.SCREEN_HEIGHT + camera_y,
                                                         random_range(20, 40), color_random, 0, thickness,
                                                         0.1 * random_range(1, 20), speed_y));
    }
}
...
bool Player::game_mode_is_multiplayer () {
    ...
    if (game_in_progress) {
        if (game_mode == GAME_MODE_MP_ADVENTURE || game_mode == GAME_MODE_MP_SURVIVAL ||
            game_mode == GAME_MODE_MP_VERSUS) {
            multiplayer = true;
        }
    }
...
bool Player::all_players_gone () {
    ...
    for (int i = 0; i < mp_players.size(); i++) {
        if ((mp_players[i].DYING && !collision_check(mp_players[i].x, mp_players[i].y, mp_players[i].w, mp_players[i].h,
                                                     camera_x, camera_y, camera_w,
                                                     camera_h)) || mp_players[i].bubble_mode) {
            players_gone++;
        }
    }
...
void Player::mp_reset (double new_x, double new_y) {
    ...
    air_velocity = ((double) random_range((uint32_t) (jump_min * 10.0),
                                          (uint32_t) (jump_max * 0.75 * 10.0)) / 10.0) * -1.0;
    IN_AIR = true;
    frame_jump = 0;

    if (random_range(0, 99) < 50) {
        run_speed = ((double) random_range((uint32_t) (max_speed / 4.0 * 10.0),
                                           (uint32_t) (max_speed / 2.0 * 10.0)) / 10.0) * -1.0;
    } else {
        run_speed = (double) random_range((uint32_t) (max_speed / 4.0 * 10.0),
                                          (uint32_t) (max_speed / 2.0 * 10.0)) / 10.0;
    }
}
                                ...
                                for (int x = 0; x < candy_width; x++) {
                                    for (int y = 0; y < candy_height; y++) {
                                        vector_items.push_back(Item(vector_items[i].x + x * ITEM_W,
                                                                    vector_items[i].y + y * ITEM_H, false, ITEM_CANDY,
                                                                    0, false));
                                    }
                                }
                            }
