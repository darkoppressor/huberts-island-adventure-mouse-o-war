...
void File_IO_Load::open (string path, bool path_is_backup, bool get_binary, bool suppress_errors) {
    ...
    if (rwops != 0) {
        ...
        if (SDL_RWclose(rwops) != 0 && !suppress_errors) {
            msg = "Error closing file '" + path + "' after loading: ";
            msg += SDL_GetError();
            update_error_log(msg, false);
        }
    } else {
        if (!suppress_errors) {
            msg = "Error opening file '" + path + "' for loading: ";
            msg += SDL_GetError();
            update_error_log(msg, false);
        }
...
void File_IO_Save::open (string get_path, bool append, bool binary) {
    ...
    if (!is_opened()) {
        msg = "Error opening file '" + path + "' for saving: ";
        msg += SDL_GetError();
        update_error_log(msg, false);
    }
}
...
bool File_IO_Save::close () {
    ...
    if (close_result != 0) {
        msg = "Error closing file '" + path + "' after saving: ";
        msg += SDL_GetError();
        update_error_log(msg, false);
...
bool File_IO::save_atomic (string path, string data, bool backup, bool append, bool binary) {
    ...
    bool File_IO::is_directory (string path) {
        ...
        if (stat(path.c_str(), &file_info) == 0) {
            ...
            update_error_log("Error getting file information for file '" + path + "': " + num_to_string(errno), false);
        }
    ...
    bool File_IO::rename_file (string old_path, string new_path) {
        if (is_regular_file(old_path)) {
            if (rename(old_path.c_str(), new_path.c_str()) != 0) {
                update_error_log("Error renaming file '" + old_path + "' to '" + new_path + "': " +
                                 num_to_string(errno), false);
            ...
            if (!exists(new_path)) {
                ...
                if (load.is_opened()) {
                    ...
                    if (save.is_opened()) {
                        ...
                        update_error_log("Error opening file '" + new_path + "' for copying to", false);

                        return false;
                    }
                } else {
                    update_error_log("Failed to load file for copying from: '" + old_path + "'", false);

                    return false;
                }
            } else {
                update_error_log("Error copying file to '" + new_path + "': file exists", false);
        ...
        if (exists(path)) {
            if (is_regular_file(path)) {
                if (remove(path.c_str()) != 0) {
                    update_error_log("Error removing file '" + path + "': " + num_to_string(errno), false);
    ...
    bool File_IO::external_storage_available () {
        ...
        if (external_storage_state != 0) {
            if ((external_storage_state & SDL_ANDROID_EXTERNAL_STORAGE_READ) != 0 &&
                ...
                update_error_log("External storage is not RW enabled.", false);
            }
        } else {
            msg = "External storage is unavailable: ";
            msg += SDL_GetError();
            update_error_log(msg, false);
        }
    ...
    bool File_IO_Directory_Iterator_User_Data::is_regular_file () {
        ...
        if (stat(get_full_path().c_str(), &file_info) == 0) {
            ...
            update_error_log("Error getting file information for file '" + get_full_path() + "': " +
                             num_to_string(errno), false);
        }
    ...
    bool File_IO::create_directory (string path) {
        try {
            if (!boost::filesystem::create_directory(path)) {
                update_error_log("Error creating directory '" + path +
    ...
    bool File_IO::rename_file (string old_path, string new_path) {
        try {
            ...
            update_error_log("Error renaming file '" + old_path + "': " + error_message, false);

            return false;
        } catch (...) {
            update_error_log("Error renaming file '" + old_path + "'", false);
        ...
        if (is_regular_file(old_path)) {
            if (!exists(new_path)) {
                try {
                    ...
                    update_error_log("Error copying file '" + old_path + "': " + error_message, false);

                    return false;
                } catch (...) {
                    update_error_log("Error copying file '" + old_path + "'", false);
