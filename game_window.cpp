...
bool Game_Window::initialize_video () {
    ...
    if (player.option_fullscreen) {
        if (player.option_fullscreen_mode == "desktop") {
            screen = SDL_CreateWindow("Hubert's Island Adventure: Mouse o' War", position_x, position_y, 0, 0,
            ...
            if (player.option_fullscreen_mode == "standard") {
                if (display_res_max.w > -1 && display_res_max.h > -1) {
                    if (window_width > display_res_max.w || window_height > display_res_max.h) {
                        update_error_log("Window dimensions of " + num_to_string(window_width) + "x" +
                                         num_to_string(window_height) + " exceed the maximum display dimensions of " +
                                         num_to_string(display_res_max.w) + "x" + num_to_string(display_res_max.h) +
            ...
            if (display_modes >= 1) {
                for (int i = 0; i < display_modes; i++) {
                    ...
                    if (SDL_GetDisplayMode(display_index, i, &mode) == 0) {
                        ...
                        msg = "Unable to get display mode #" + num_to_string(i) + " for display #" +
                              num_to_string(display_index) + ": ";
                        msg += SDL_GetError();
                        update_error_log(msg);
                    }
                }
                        ...
                        if (display_res_max.w > -1 && display_res_max.h > -1) {
                            if (*desired_resolution_x > display_res_max.w ||
                                *desired_resolution_y > display_res_max.h) {
                                update_error_log("Window dimensions of " + num_to_string(*desired_resolution_x) + "x" +
                                                 num_to_string(*desired_resolution_y) +
                                                 " exceed the maximum display dimensions of " +
                                                 num_to_string(display_res_max.w) + "x" +
                                                 num_to_string(display_res_max.h) +
    ...
    if (!initialized) {
        ...
        if (Mix_Init(MIX_INIT_OGG) == 0) {
            ...
            if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 1024) == -1) {
                ...
                if (channels_allocated != channels_requested) {
                    msg = "Error allocating mixer channels: Requested " + num_to_string(channels_requested) +
                          ", allocated " + num_to_string(channels_allocated) + ".";
                    update_error_log(msg);
                }
            }
        }
        ...
        for (int int_x = 0; int_x < 16; int_x++) {
            for (int int_y = 0; int_y < 16; int_y++) {
                ...
                SDL_GetRGBA(surface_get_pixel(temp_surface, int_x, int_y), temp_surface->format, &check_red,
                            &check_green, &check_blue, &check_alpha);
...
void Game_Window::screenshot () {
    ...
    if (player.name != "\x1F") {
        ...
        if (pixel_data != 0) {
            ...
            SDL_Surface* surface = SDL_CreateRGBSurfaceFrom(pixel_data, actual_width, actual_height, 32,
                                                            actual_width * 4, rmask, gmask, bmask, amask);
