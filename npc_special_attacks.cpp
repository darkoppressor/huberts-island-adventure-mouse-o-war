...
bool Npc::water_touching_this_tile (int int_x, int int_y) {
    ...
    if (int_x > 0 && int_y < (level.level_y / TILE_SIZE) - 1 &&
        level.tile_array[int_x - 1][int_y + 1].special == TILE_SPECIAL_WATER) {
        return true;
    }

    if (int_x < (level.level_x / TILE_SIZE) - 1 && int_y < (level.level_y / TILE_SIZE) - 1 &&
        level.tile_array[int_x + 1][int_y + 1].special == TILE_SPECIAL_WATER) {
        return true;
    }

    if (int_x < (level.level_x / TILE_SIZE) - 1 && int_y > 0 &&
        level.tile_array[int_x + 1][int_y - 1].special == TILE_SPECIAL_WATER) {
        return true;
    }
...
void Npc::ai_special_attack_bee_divebomb () {
    ...
    if (ai_notice_wall() || collision_check(x, y, w, h, special_attack_target_x + PLAYER_W / 2 - 2,
                                            special_attack_target_y + PLAYER_H / 2 - 2, 4, 4)) {
        returning_home = true;
    }
...
void Npc::ai_special_attack_replicate () {
    for (int i = 0; i < vector_doors.size(); i++) {
        if (fabs(vector_doors[i].x - x) < PROCESS_RANGE && fabs(vector_doors[i].y - y) < PROCESS_RANGE) {
            if (!vector_doors[i].open && collision_check(x - TILE_SIZE, y - TILE_SIZE, TILE_SIZE * 3, TILE_SIZE * 3,
                                                         vector_doors[i].x, vector_doors[i].y, vector_doors[i].w,
                                                         vector_doors[i].h)) {
        ...
        for (int i = 0; i < vector_npcs.size(); i++) {
            if (this != &vector_npcs[i]) {
                if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                    if (vector_npcs[i].exists && vector_npcs[i].type != type && collision_check(x - TILE_SIZE,
                                                                                                y - TILE_SIZE,
                                                                                                TILE_SIZE * 3,
                                                                                                TILE_SIZE * 3,
                                                                                                vector_npcs[i].x,
    ...
    if (!replicate) {
        for (int i = 0; i < vector_traps.size(); i++) {
            if (fabs(vector_traps[i].x - x) < PROCESS_RANGE && fabs(vector_traps[i].y - y) < PROCESS_RANGE) {
                if (vector_traps[i].active && collision_check(x - TILE_SIZE, y - TILE_SIZE, TILE_SIZE * 3,
                                                              TILE_SIZE * 3, vector_traps[i].x, vector_traps[i].y,
                                                              vector_traps[i].w, vector_traps[i].h)) {
    ...
    if (!replicate) {
        for (int i = 0; i < vector_items.size(); i++) {
            if (fabs(vector_items[i].x - x) < PROCESS_RANGE && fabs(vector_items[i].y - y) < PROCESS_RANGE) {
                if (vector_items[i].exists && collision_check(x - TILE_SIZE, y - TILE_SIZE, TILE_SIZE * 3,
                                                              TILE_SIZE * 3, vector_items[i].x, vector_items[i].y,
                                                              vector_items[i].w, vector_items[i].h)) {
        ...
        for (int int_x = check_x_start; !replicate && int_x < check_x_end; int_x++) {
            ...
            if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                ...
                if (!water_touching_this_tile(int_x, int_y)) {
                    if (level.tile_array[int_x][int_y].sprite != 0 ||
...
void Npc::ai_special_attack_eagle_divebomb () {
    ...
    // If the target is reached, or the npc is giving up, begin returning home.
    if (ai_notice_wall() || collision_check(x, y, w, h, PLAYER_X + PLAYER_WIDTH / 2 - 2,
                                            PLAYER_Y + PLAYER_HEIGHT / 2 - 2, 4, 4)) {
        returning_home = true;
    }
...
void Npc::ai_special_attack_sphinx_suck_left () {
    ...
    if (player.x + player.w < x && collision_check(suck_x, suck_y, suck_w, suck_h, player.x, player.y, player.w,
                                                   player.h)) {
    ...
    for (int i = 0; i < mp_players.size(); i++) {
        if (mp_players[i].x + mp_players[i].w < x && collision_check(suck_x, suck_y, suck_w, suck_h, mp_players[i].x,
                                                                     mp_players[i].y, mp_players[i].w,
                                                                     mp_players[i].h)) {
    ...
    for (int i = 0; i < vector_npcs.size(); i++) {
        if (this != &vector_npcs[i]) {
            if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                if (vector_npcs[i].x + vector_npcs[i].w < x && collision_check(suck_x, suck_y, suck_w, suck_h,
                                                                               vector_npcs[i].x, vector_npcs[i].y,
                                                                               vector_npcs[i].w, vector_npcs[i].h)) {
    ...
    for (int i = 0; i < vector_shots.size(); i++) {
        if (fabs(vector_shots[i].x - x) < PROCESS_RANGE && fabs(vector_shots[i].y - y) < PROCESS_RANGE) {
            if (vector_shots[i].x + vector_shots[i].w < x && collision_check(suck_x, suck_y, suck_w, suck_h,
                                                                             vector_shots[i].x, vector_shots[i].y,
                                                                             vector_shots[i].w, vector_shots[i].h)) {
...
void Npc::ai_special_attack_sphinx_suck_right () {
    ...
    if (player.x > x + w && collision_check(suck_x, suck_y, suck_w, suck_h, player.x, player.y, player.w, player.h)) {
        player.sucked_left = true;
        anything_sucked = true;
    }

    for (int i = 0; i < mp_players.size(); i++) {
        if (mp_players[i].x > x + w && collision_check(suck_x, suck_y, suck_w, suck_h, mp_players[i].x, mp_players[i].y,
                                                       mp_players[i].w, mp_players[i].h)) {
    ...
    for (int i = 0; i < vector_npcs.size(); i++) {
        if (this != &vector_npcs[i]) {
            if (fabs(vector_npcs[i].x - x) < PROCESS_RANGE && fabs(vector_npcs[i].y - y) < PROCESS_RANGE) {
                if (vector_npcs[i].x > x + w && collision_check(suck_x, suck_y, suck_w, suck_h, vector_npcs[i].x,
                                                                vector_npcs[i].y, vector_npcs[i].w, vector_npcs[i].h)) {
    ...
    for (int i = 0; i < vector_shots.size(); i++) {
        if (fabs(vector_shots[i].x - x) < PROCESS_RANGE && fabs(vector_shots[i].y - y) < PROCESS_RANGE) {
            if (vector_shots[i].x > x + w && collision_check(suck_x, suck_y, suck_w, suck_h, vector_shots[i].x,
                                                             vector_shots[i].y, vector_shots[i].w, vector_shots[i].h)) {
