        ...
        for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
            ...
            if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                ...
                if ((level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID ||
                    ...
                    // First, we check for an x-axis collision. For x-axis collision, we are checking this part of the
                    // player's rectangle:
                    // |----|
                    // xxxxxx
                    // xxxxxx
                    ...
                    if (collision_check(x, y + 5, w, h - 10, level.tile_array[int_x][int_y].x,
                        ...
                        // If the left of the player is left of the tile's left side, we know the player is hitting the
                    ...
                    // |-xx-|
                                             ...
                                             level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                        // If the top of the player is above the tile's top side, we know the player is hitting the top
                        // side of the tile.
                                ...
                                if (level.tile_array[int_x][int_y].slope == 45) {
                                    ...
                                    if (direction == DOWN &&
                                        ...
                                        touching_sloped_ground = true;
                                               ...
                                               y <=
                                               level.tile_array[int_x][int_y].y + TILE_SIZE - TILE_SIZE + x_position) {
                                        // Snap to the slope.
...
void Player_Mp::worldmap_render () {
    if (move_state == 0 && (facing == LEFT || facing == LEFT_UP || facing == LEFT_DOWN)) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_idle_left[frame_idle]);
    } else if (move_state == 0 && (facing == RIGHT || facing == RIGHT_UP || facing == RIGHT_DOWN)) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_idle_right[frame_idle]);
    } else if (move_state == 0 && facing == UP) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_idle_up[frame_idle]);
    } else if (move_state == 0 && facing == DOWN) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_idle_down[frame_idle]);
    } else if (move_state != 0 && (facing == LEFT || facing == LEFT_UP || facing == LEFT_DOWN)) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_walk_left[frame]);
    } else if (move_state != 0 && (facing == RIGHT || facing == RIGHT_UP || facing == RIGHT_DOWN)) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_walk_right[frame]);
    } else if (move_state != 0 && facing == UP) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_walk_up[frame]);
    } else if (move_state != 0 && facing == DOWN) {
        render_sprite((int) ((int) x - (int) player.camera_x), (int) ((int) y - (int) player.camera_y),
                      *ptr_player_worldmap_image, &sprites_player_worldmap_walk_down[frame]);
    }
