...
void Window_Map::handle_map_input () {
    ...
    if (player.command_state(COMMAND_LEFT) && player.command_state(COMMAND_DOWN) &&
        player.command_state(COMMAND_RIGHT)) {
        cam_state = LEFT_DOWN;
    }
    ...
    if (player.command_state(COMMAND_LEFT) && player.command_state(COMMAND_UP) && player.command_state(COMMAND_RIGHT) &&
        player.command_state(COMMAND_DOWN)) {
        cam_state = LEFT_UP;
    }

    // If no camera directional keys are pressed, stop the camera.
    if (!player.command_state(COMMAND_LEFT) && !player.command_state(COMMAND_UP) &&
        !player.command_state(COMMAND_RIGHT) && !player.command_state(COMMAND_DOWN)) {
        cam_state = 0;
    }
...
void Window_Map::set_map_level (short get_level, short get_sub_level) {
    if (get_level == player.current_level && get_sub_level == player.current_sub_level &&
        player.persistent_level_data) {
        profile.save_map();
    }
...
void Window_Map::load_map_data () {
    if (persistent_level_data) {
        ...
        load_path = profile.get_home_directory() + "profiles/" + player.name + "/saves/" + current_level +
                    "/map.blazesave";
        load.open(load_path);
            ...
            if (load.is_opened()) {
                ...
                for (int i = 0; i < viewable_items.size(); i++) {
                    ...
                    if (profile.is_item_collectable(viewable_items[i].type)) {
                        ...
                        if (viewable_items[i].type != ITEM_SPAWNPOINT && viewable_items[i].type != ITEM_CHECKPOINT &&
                            viewable_items[i].type != ITEM_ENDPOINT) {
                            viewable_items[i].exists = false;
                        }
                    }
                }
...
void Window_Map::display_map () {
    ...
    if (persistent_level_data) {
        ...
        for (int int_y = 0; int_y < level_y / TILE_SIZE; int_y++) {
            for (int int_x = 0; int_x < level_x / TILE_SIZE; int_x++) {
                ...
                if (int_x >= 0 && int_x <= (level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                    int_y <= (level_y / TILE_SIZE) - 1 && int_x * 16 >= map_camera_x - 16 &&
                    int_x * 16 <= map_camera_x + map_camera_w && int_y * 16 >= map_camera_y - 16 &&
                    int_y * 16 <= map_camera_y + map_camera_h && (int) (int_x * 16 - map_camera_x) >= x + 10 &&
                    (int) (int_x * 16 + TILE_SIZE / 2 - map_camera_x) <= x + 10 + map_camera_w &&
                    35 + (int) (int_y * 16 - map_camera_y) >= y + 35 &&
                    35 + (int) (int_y * 16 + TILE_SIZE / 2 - map_camera_y) <= y + 35 + map_camera_h) {
                    if (viewable_map_data[int_x][int_y].seen) {
                        ...
                        if (player.is_level_worldmap(current_viewable_level)) {
                            if (viewable_map_data[int_x][int_y].sprite != 0) {
                                ...
                                if (viewable_map_data[int_x][int_y].sprite == 1 ||
                                    viewable_map_data[int_x][int_y].sprite == 2) {
                                    render_sprite((int) ((int) int_x * 16 - (int) map_camera_x),
                                                  35 + (int) ((int) int_y * 16 - (int) map_camera_y),
                                                  viewable_water_tiles,
                                                  &sprites_tiles[viewable_map_data[int_x][int_y].sprite], 0.75, 0.5,
                                                  0.5);
                                } else {
                                    render_sprite((int) ((int) int_x * 16 - (int) map_camera_x),
                                                  35 + (int) ((int) int_y * 16 - (int) map_camera_y), viewable_tiles,
                                                  &worldmap_sprites_tiles[viewable_map_data[int_x][int_y].sprite], 1.0,
                                                  0.5, 0.5);
                                }
                            }
                        }
                        // If the current level is not the world map.
                            ...
                            if (viewable_map_data[int_x][int_y].background_sprite != 0) {
                                render_sprite((int) ((int) int_x * 16 - (int) map_camera_x),
                                              35 + (int) ((int) int_y * 16 - (int) map_camera_y),
                                              viewable_background_tiles,
                                              &sprites_tiles_background[viewable_map_data[int_x][int_y].
                                                                        background_sprite], 1.0, 0.5, 0.5);
                            }
                            ...
                            if (viewable_map_data[int_x][int_y].sprite != 0) {
                                ...
                                if (viewable_map_data[int_x][int_y].sprite == 1 ||
                                    ...
                                    render_sprite((int) ((int) int_x * 16 - (int) map_camera_x),
                                                  35 + (int) ((int) int_y * 16 - (int) map_camera_y), viewable_tiles,
                                                  &sprites_tiles[viewable_map_data[int_x][int_y].sprite], 1.0, 0.5,
                                                  0.5);
                                }
                            }
                        }
                    } else {
                        render_rectangle((int) ((int) int_x * 16 - (int) map_camera_x),
                                         35 + (int) ((int) int_y * 16 - (int) map_camera_y), 16, 16, 1.0, COLOR_BLACK);
                    }
                }
            }
        }
            ...
            for (int i = 0; i < viewable_items.size(); i++) {
                if (viewable_items[i].exists) {
                    ...
                    for (int int_y = check_y_start; int_y <= check_y_end && !touching_seen_tile; int_y++) {
                        for (int int_x = check_x_start; int_x <= check_x_end && !touching_seen_tile; int_x++) {
                            if (int_x >= 0 && int_x <= (level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                                ...
                                if (viewable_map_data[int_x][int_y].seen && collision_check(viewable_items[i].x,
                                                                                            viewable_items[i].y,
                                                                                            viewable_items[i].w,
                                                                                            viewable_items[i].h,
                                                                                            int_x * TILE_SIZE,
                    ...
                    if (touching_seen_tile) {
                        if ((int) (viewable_items[i].x / 2 - map_camera_x) >= x + 10 &&
                            (int) (viewable_items[i].x / 2 + viewable_items[i].w / 2 - map_camera_x) <=
                            x + 10 + map_camera_w && 35 + (int) (viewable_items[i].y / 2 - map_camera_y) >= y + 35 &&
                            35 + (int) (viewable_items[i].y / 2 + viewable_items[i].h / 2 - map_camera_y) <=
                            y + 35 + map_camera_h) {
                            image_data item_image = image.sprite_sheet_items;

                            if (viewable_items[i].type == ITEM_SPAWNPOINT ||
                                viewable_items[i].type == ITEM_CHECKPOINT || viewable_items[i].type == ITEM_ENDPOINT) {
                            ...
                            render_sprite((int) ((int) viewable_items[i].x / 2 - (int) map_camera_x),
                                          35 + (int) (((int) viewable_items[i].y + y_adjust) / 2 - (int) map_camera_y),
                                          item_image, viewable_items[i].get_texture_clip(true), 1.0, 0.5, 0.5, 0.0,
                                          viewable_items[i].color);
                        }
                    }
                }
            }
        ...
        if (current_viewable_level == player.current_level && current_viewable_sub_level == player.current_sub_level) {
            ...
            if ((int) (player.x / 2 - map_camera_x) >= x + 10 &&
                (int) (player.x / 2 + player.w / 2 - map_camera_x) <= x + 10 + map_camera_w &&
                35 + (int) (player.y / 2 - map_camera_y) >= y + 35 &&
                35 + (int) (player.y / 2 + player.h / 2 - map_camera_y) <= y + 35 + map_camera_h) {
                // If the current level is the world map.
                if (player.is_level_worldmap(current_viewable_level)) {
                    render_sprite((int) ((int) player.x / 2 - (int) map_camera_x),
                                  35 + (int) ((int) player.y / 2 - (int) map_camera_y),
                                  image.sprite_sheet_player_worldmap, &sprites_player_worldmap_idle_right[0], 1.0, 0.5,
                                  0.5);
                }
                // If the current level is not the world map.
                else {
                    render_sprite((int) ((int) player.x / 2 - (int) map_camera_x),
                                  35 + (int) ((int) player.y / 2 - (int) map_camera_y), image.sprite_sheet_player,
                                  &sprites_player_idle_right[0], 1.0, 0.5, 0.5);
                }
            }
        }
    } else {
