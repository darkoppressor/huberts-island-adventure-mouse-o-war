...
Item::Item (double get_x, double get_y, bool get_IN_AIR, short get_type, int get_goal_level_to_load,
            bool get_goal_secret, int min_move, int max_move, int min_jump, int max_jump, bool can_start_falling,
            short get_counter_vacuum, int get_score_bonus) {
...
void Item::move () {
    if (exists) {
        ...
        if (!PLAYER_DYING && !PLAYER_BUBBLE_MODE && counter_cannot_be_vacuumed == 0 && type == ITEM_CANDY &&
            fabs(PLAYER_X - x) < candy_vac_range && fabs(PLAYER_Y - y) < candy_vac_range) {
...
void Item::handle_events () {
    ...
    if (exists) {
        ...
        if (IN_AIR || SWIMMING) {
                ...
                for (int int_x = check_x_start; int_x < check_x_end; int_x++) {
                    if (int_x >= 0 && int_x <= (level.level_x / TILE_SIZE) - 1 && int_y >= 0 &&
                        ...
                        if (SWIMMING && level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_PASSABLE &&
                            ...
                            if (collision_check(x, y, w, h, level.tile_array[int_x][int_y].x,
                        ...
                        if (!SWIMMING && level.tile_array[int_x][int_y].special == TILE_SPECIAL_WATER) {
                            if (collision_check(x, y, w, h, level.tile_array[int_x][int_y].x,
                                                level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                                float_speed = max_buoyancy;
                                // Move the actor just below the top of this water tile, so he doesn't "rehit" the air
                                // above the water.
                        ...
                        if (level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_SOLID) {
                            // First, we check for an x-axis collision. For x-axis collision, we are checking this part
                            // of the item's rectangle:
                            // |----|
                            // xxxxxx
                            // xxxxxx
                            // |----|
                            if (collision_check(x, y + (h / 4), w, h / 2, level.tile_array[int_x][int_y].x,
                                                level.tile_array[int_x][int_y].y, TILE_SIZE, TILE_SIZE)) {
                                // If the left of the item is left of the tile's left side, we know the item is hitting
                                // the left side of the tile.
                                if (x < level.tile_array[int_x][int_y].x) {
                                    // We set the item's x value to the x value of the tile - the item's width, so the
                                    // item will now be on the left side of the tile.
                                    x = level.tile_array[int_x][int_y].x - w;

                                    // Since the item just bumped into the left side of the tile, it must be moving
                                    // right, so start it moving left.
                                    move_state = LEFT;

                                    run_speed *= -1.0;
                                }
                                // If the right side of the item is right of the tile's right side, we know the item is
                                // hitting the right side of the tile.
                                    ...
                                    // Since the item just bumped into the right side of the tile, it must be moving
                                    // left, so start it moving right.
                                    move_state = RIGHT;
                            ...
                            // Now we check to see if there is a y-axis collision. The section of the item that collides
                            // with the solid tile matters.
                            ...
                            if ((air_velocity < 0.0 || float_speed < 0.0) && collision_check(x, y, w, h,
                                                                                             level.tile_array[int_x][
                                                                                                 int_y].x,
                                                                                             level.tile_array[int_x][
                                                                                                 int_y].y + TILE_SIZE /
                                                                                             2, TILE_SIZE,
                                ...
                                // We set the air velocity to 0, so that the item immediately begins falling once it
                                // bumps its top on a tile.
                                air_velocity = 0.0;
                                float_speed = 0.0;
                            }
                            ...
                            // |----|
                            // xxxxxx
                            // We only do this if the item is falling, so that it doesn't get warped to the top of the
                            // tile if jumping into the side of it.
                            if ((air_velocity >= 0.0 || float_speed >= 0.0) && collision_check(x, y + h - (h / 2), w,
                                                                                               h / 2,
                                                                                               level.tile_array[int_x][
                                                                                                   int_y].x,
                                                                                               level.tile_array[int_x][
                                                                                                   int_y].y, TILE_SIZE,
                                                                                               TILE_SIZE / 2)) {
                                // We set the item's y value to the y value of the tile - the item's height, so the item
                                // will now be right on top of the tile.
                        ...
                        if ((level.tile_array[int_x][int_y].solidity == TILE_SOLIDITY_CLOUD ||
                            ...
                            // xxxxxx
                            // |----|
                            // If there is an x-axis collision while the item is falling, don't check for a y-axis
                            // collision this time.
                            // This prevents a falling item from warping back to the top of this tile, which would be
                            // caused by the below if() statement alone.
                            ...
                            // |----|
                            // xxxxxx
                            // As shown above, we are checking only the bottom row of the pixels for the item against
                            // only the top row of the pixels for the tile.
                            if (collision_check(x, y + h - (h / 2), w, h / 2, level.tile_array[int_x][int_y].x,
            ...
            if (y < 0) {
                y = 0;
                // We set the air velocity to 0, so that the item immediately begins falling once it bumps its top on
            ...
            // If the player is in a level.
                ...
                if (type == ITEM_LEAF) {
                    render_sprite((int) ((int) render_x - (int) player.camera_x),
                                  (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_items,
                                  get_texture_clip(survival_spawn), 1.0, 1.0, 1.0, 0.0, color);
                } else if (type == ITEM_CHEESE) {
                    render_sprite((int) ((int) render_x - (int) player.camera_x),
                                  (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_items,
                                  get_texture_clip(survival_spawn), 1.0, 1.0, 1.0, 0.0, color);
                    ...
                    if (survival_spawn) {
                        if (player.game_mode == GAME_MODE_SP_ADVENTURE || player.game_mode == GAME_MODE_MP_ADVENTURE) {
                            render_sprite((int) ((int) render_x - (int) player.camera_x),
                                          (int) ((int) render_y - 32 - (int) player.camera_y),
                                          image.sprite_sheet_level_items, get_texture_clip(survival_spawn), 1.0, 1.0,
                                          1.0, 0.0, color);
                        } else if (player.game_mode == GAME_MODE_SP_SURVIVAL ||
                                   player.game_mode == GAME_MODE_MP_SURVIVAL) {
                            if (player.survival_escape) {
                                render_sprite((int) ((int) render_x - (int) player.camera_x),
                                              (int) ((int) render_y - 32 - (int) player.camera_y), image.survival_spawn,
                                              get_texture_clip(survival_spawn), 1.0, 1.0, 1.0, 0.0, color);
                            } else {
                                render_sprite((int) ((int) render_x - (int) player.camera_x),
                                              (int) ((int) render_y - 32 - (int) player.camera_y), image.survival_spawn,
                    ...
                    render_sprite((int) ((int) render_x - (int) player.camera_x),
                                  (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_level_items,
                                  get_texture_clip(survival_spawn), 1.0, 1.0, 1.0, 0.0, color);
                } else if (type == ITEM_ENDPOINT) {
                    if (!goal_secret) {
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                    ...
                    if (color != COLOR_BLACK) {
                        ...
                        render_sprite((int) ((int) render_x - (int) player.camera_x),
                                      (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_items,
                                      get_texture_clip(survival_spawn));
                    }
                } else if (type == ITEM_SUIT_DEADLY_WATER) {
                    render_sprite((int) ((int) render_x - (int) player.camera_x),
                                  (int) ((int) render_y - (int) player.camera_y), image.sprite_sheet_items,
